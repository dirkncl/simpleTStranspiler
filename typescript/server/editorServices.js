...
export const CreateFileWatcherEvent = "createFileWatcher";
export const CreateDirectoryWatcherEvent = "createDirectoryWatcher";
export const CloseFileWatcherEvent = "closeFileWatcher";
...
export function convertFormatOptions(protocolOptions) {
...
export function convertCompilerOptions(protocolOptions) {
...
export function convertWatchOptions(protocolOptions, currentDirectory) {
    let watchOptions;
    let errors;
...
export function convertTypeAcquisition(protocolOptions) {
    let result;
...
export function tryConvertScriptKindName(scriptKindName) {
...
/** @internal */
export function convertUserPreferences(preferences) {
...
export const nullTypingsInstaller = {
...
const noopConfigFileWatcher = { close: noop };
...
function getConfigFileNameFromCache(info, cache) {
    ...
    if (configFileForOpenFile === undefined)
        return undefined;
...
/** @internal */
export var ConfiguredProjectLoadKind;
(function (ConfiguredProjectLoadKind) {
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["FindOptimized"] = 0] = "FindOptimized";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["Find"] = 1] = "Find";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["CreateReplayOptimized"] = 2] = "CreateReplayOptimized";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["CreateReplay"] = 3] = "CreateReplay";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["CreateOptimized"] = 4] = "CreateOptimized";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["Create"] = 5] = "Create";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["ReloadOptimized"] = 6] = "ReloadOptimized";
    ConfiguredProjectLoadKind[ConfiguredProjectLoadKind["Reload"] = 7] = "Reload";
 ...
 */
function forEachAncestorProjectLoad(info, project, cb, kind, 
...
delayReloadedConfiguredProjects) {
    ...
    while (true) {
        ...
        const ancestor = project.projectService.findCreateOrReloadConfiguredProject(configFileName, kind, reason, allowDeferredClosed, !searchOnlyPotentialSolution ? info.fileName : undefined, // Config Diag event for project if its for default project
        reloadedProjects, searchOnlyPotentialSolution, // Delay load if we are searching for solution
        delayReloadedConfiguredProjects);
        if (!ancestor)
            return;
        // If this ancestor is new and was delay loaded, then set the project as potential project reference
        if (!ancestor.project.parsedCommandLine &&
            ((_a = project.parsedCommandLine) === null || _a === void 0 ? void 0 : _a.options.composite)) {
 ...
 */
function forEachResolvedProjectReferenceProjectLoad(project, parentConfig, cb, kind, reason, 
/** Used with ConfiguredProjectLoadKind.Find to get deferredClosed projects as well */
allowDeferredClosed, 
/** Used with ConfiguredProjectLoadKind.Reload to check if this project was already reloaded */
reloadedProjects, seenResolvedRefs) {
    const loadKind = parentConfig.options.disableReferencedProjectLoad ? ConfiguredProjectLoadKind.FindOptimized : kind;
    let children;
    return forEach(parentConfig.projectReferences, ref => {
        var _a, _b;
        ...
        const seenValue = seenResolvedRefs === null || seenResolvedRefs === void 0 ? void 0 : seenResolvedRefs.get(childCanonicalConfigPath);
        ...
        let childConfig = loadKind === ConfiguredProjectLoadKind.FindOptimized ?
            (configFileExistenceInfo === null || configFileExistenceInfo === void 0 ? void 0 : configFileExistenceInfo.exists) || ((_a = project.resolvedChildConfigs) === null || _a === void 0 ? void 0 : _a.has(childCanonicalConfigPath)) ?
        ...
        if (loadKind === ConfiguredProjectLoadKind.CreateReplayOptimized &&
        ...
        switch (loadKind) {
            case ConfiguredProjectLoadKind.ReloadOptimized:
                if (childProject)
                    childProject.projectService.reloadConfiguredProjectOptimized(childProject, reason, reloadedProjects);
            // falls through
            case ConfiguredProjectLoadKind.CreateOptimized:
                ((_b = project.resolvedChildConfigs) !== null && _b !== void 0 ? _b : (project.resolvedChildConfigs = new Set())).add(childCanonicalConfigPath);
            // falls through
            ...
            case ConfiguredProjectLoadKind.FindOptimized:
                if (childProject || loadKind !== ConfiguredProjectLoadKind.FindOptimized) {
                    const result = cb(configFileExistenceInfo !== null && configFileExistenceInfo !== void 0 ? configFileExistenceInfo : project.projectService.configFileExistenceInfoCache.get(childCanonicalConfigPath), childProject, childConfigName, reason, project, childCanonicalConfigPath);
                    if (result)
                        return result;
                }
                break;
        ...
        (seenResolvedRefs !== null && seenResolvedRefs !== void 0 ? seenResolvedRefs : (seenResolvedRefs = new Map())).set(childCanonicalConfigPath, loadKind);
        (children !== null && children !== void 0 ? children : (children = [])).push(childConfig);
    }) || forEach(children, childConfig => childConfig.projectReferences && forEachResolvedProjectReferenceProjectLoad(project, childConfig, cb, loadKind, reason, allowDeferredClosed, reloadedProjects, seenResolvedRefs));
}
function updateProjectFoundUsingFind(project, kind, 
...
reloadedProjects) {
    ...
    switch (kind) {
        ...
        case ConfiguredProjectLoadKind.Reload:
            sentConfigFileDiag = project.projectService.reloadConfiguredProjectClearingSemanticCache(project, reason, reloadedProjects);
            break;
        case ConfiguredProjectLoadKind.FindOptimized:
...
function forEachPotentialProjectReference(project, cb) {
    ...
    return project.initialLoadPending ?
        (_a = (project.potentialProjectReferences && forEachKey(project.potentialProjectReferences, cb))) !== null && _a !== void 0 ? _a : (project.resolvedChildConfigs && forEachKey(project.resolvedChildConfigs, cb)) :
        undefined;
}
...
function forEachReferencedProject(project, cb) {
    return forEachAnyProjectReferenceKind(project, resolvedRef => callbackRefProject(project, cb, resolvedRef.sourceFile.path), projectRef => callbackRefProject(project, cb, project.toPath(resolveProjectReferencePath(projectRef))), potentialProjectRef => callbackRefProject(project, cb, potentialProjectRef));
}
/** @internal */
export function getDetailWatchInfo(watchType, project) {
 ...
 * @internal
 */
export function updateProjectIfDirty(project) {
...
function updateWithTriggerFile(project, triggerFile, isReload) {
    ...
    if (!project.triggerFileForConfigFileDiag && !isReload)
        return updateLevel === ProgramUpdateLevel.Full;
...
function updateConfiguredProject(project, triggerFile) {
    if (triggerFile) {
        if (updateWithTriggerFile(project, triggerFile, /*isReload*/ false))
            return true;
    }
    else {
...
function configFileExistenceInfoForOptimizedLoading(project) {
    ...
    const configFileExistenceInfo = project.projectService.ensureParsedConfigUptoDate(configFileName, project.canonicalConfigFilePath, project.projectService.configFileExistenceInfoCache.get(project.canonicalConfigFilePath), project);
...
function createWatchFactoryHostUsingWatchEvents(service, canUseWatchEvents) {
    if (!getCanUseWatchEvents(service, canUseWatchEvents))
        return undefined;
    ...
    function watchFile(path, callback) {
        return getOrCreateFileWatcher(watchedFiles, path, callback, id => ({ eventName: CreateFileWatcherEvent, data: { id, path } }));
    }
    function watchDirectory(path, callback, recursive) {
        return getOrCreateFileWatcher(recursive ? watchedDirectoriesRecursive : watchedDirectories, path, callback, id => ({
    ...
    function getOrCreateFileWatcher({ pathToId, idToCallbacks }, path, callback, event) {
        ...
        return {
            close() {
                ...
                if (!(callbacks === null || callbacks === void 0 ? void 0 : callbacks.delete(callback)))
                    return;
                if (callbacks.size)
                    return;
    ...
    function forEachCallback(hostWatcherMap, id, eventPaths, cb) {
        var _a;
        (_a = hostWatcherMap.idToCallbacks.get(id)) === null || _a === void 0 ? void 0 : _a.forEach(callback => {
...
export class ProjectService {
    constructor(opts) {
        ...
        this.allJsFilesForOpenFileTelemetry = new Set();
        /**
        ...
        this.rootOfInferredProjects = new Set();
        /**
        ...
        /** @internal */ this.baseline = noop;
        /** @internal */ this.verifyDocumentRegistry = noop;
        /** @internal */ this.verifyProgram = noop;
        /** @internal */ this.onProjectCreation = noop;
        this.host = opts.host;
        ...
        const log = watchLogLevel !== WatchLogLevel.None ? (s => this.logger.info(s)) : noop;
        ...
        (_a = opts.incrementalVerifier) === null || _a === void 0 ? void 0 : _a.call(opts, this);
    }
    toPath(fileName) {
    ...
    updateTypingsForProject(response) {
        ...
        switch (response.kind) {
            case ActionSet:
                ...
                project.updateTypingFiles(response.compilerOptions, response.typeAcquisition, response.unresolvedImports, response.typings);
                return;
    ...
    watchTypingLocations(response) {
        var _a;
        (_a = this.findProject(response.projectName)) === null || _a === void 0 ? void 0 : _a.watchTypingLocations(response.files);
    }
    /** @internal */
    ...
    tryGetDefaultProjectForEnsuringConfiguredProjectForFile(fileNameOrScriptInfo) {
        ...
        if (!scriptInfo)
            return undefined;
        if ((_a = this.pendingOpenFileProjectUpdates) === null || _a === void 0 ? void 0 : _a.delete(scriptInfo.path)) {
            this.tryFindDefaultConfiguredProjectAndLoadAncestorsForOpenScriptInfo(scriptInfo, ConfiguredProjectLoadKind.Create);
    ...
    handleSourceMapProjects(info) {
        ...
        if (info.sourceMapFilePath) {
            if (isString(info.sourceMapFilePath)) {
                ...
                this.delayUpdateSourceInfoProjects(sourceMapFileInfo === null || sourceMapFileInfo === void 0 ? void 0 : sourceMapFileInfo.sourceInfos);
            }
            else {
    ...
    watchWildcardDirectory(directory, flags, configFileName, config) {
        let watcher = this.watchFactory.watchDirectory(directory, fileOrDirectory => this.onWildCardDirectoryWatcherInvoke(directory, configFileName, config, result, fileOrDirectory), flags, this.getWatchOptionsFromProjectWatchOptions(config.parsedCommandLine.watchOptions, getDirectoryPath(configFileName)), WatchType.WildcardDirectory, configFileName);
        const result = {
            ...
            close() {
                ...
                if (watcher) {
                    watcher.close();
                    watcher = undefined;
                    (_a = result.packageJsonWatches) === null || _a === void 0 ? void 0 : _a.forEach(watcher => {
    ...
    onWildCardDirectoryWatcherInvoke(directory, configFileName, config, wildCardWatcher, fileOrDirectory) {
        ...
        if (!(fsResult === null || fsResult === void 0 ? void 0 : fsResult.fileExists)) {
        ...
        config.projects.forEach((watchWildcardDirectories, projectCanonicalPath) => {
            ...
            if (configuredProjectForConfig !== project &&
                ...
                if (find((_a = project.getCurrentProgram()) === null || _a === void 0 ? void 0 : _a.getResolvedProjectReferences(), ref => (ref === null || ref === void 0 ? void 0 : ref.sourceFile.path) === path)) {
            ...
            if (project.pendingUpdateLevel > updateLevel)
                return;
    ...
    delayUpdateProjectsFromParsedConfigOnConfigFileChange(canonicalConfigFilePath, loadReason) {
        ...
        configFileExistenceInfo.config.projects.forEach((_watchWildcardDirectories, projectCanonicalPath) => {
            ...
            if (projectCanonicalPath === canonicalConfigFilePath) {
                ...
                if (project.initialLoadPending) {
                    (_b = (_a = this.configFileExistenceInfoCache.get(projectCanonicalPath)) === null || _a === void 0 ? void 0 : _a.openFilesImpactedByConfigFile) === null || _b === void 0 ? void 0 : _b.forEach(path => {
                        var _a, _b;
                        // Cache the existing config file info for this open file if not already done so
                        if (!((_a = this.pendingOpenFileProjectUpdates) === null || _a === void 0 ? void 0 : _a.has(path))) {
                            ((_b = this.pendingOpenFileProjectUpdates) !== null && _b !== void 0 ? _b : (this.pendingOpenFileProjectUpdates = new Map())).set(path, this.configFileForOpenFiles.get(path));
                        }
                    });
                    // Skip refresh if project is not yet loaded
                    return;
                ...
                if (this.getHostPreferences().includeCompletionsForModuleExports &&
                    find((_c = project.getCurrentProgram()) === null || _c === void 0 ? void 0 : _c.getResolvedProjectReferences(), ref => (ref === null || ref === void 0 ? void 0 : ref.sourceFile.path) === path)) {
                    project.markAutoImportProviderAsDirty();
                }
            }
        });
    ...
    onConfigFileChanged(configFileName, canonicalConfigFilePath, eventKind) {
        ...
        const wasDefferedClose = project === null || project === void 0 ? void 0 : project.deferredClose;
        if (eventKind === FileWatcherEventKind.Deleted) {
        ...
        this.openFiles.forEach((_projectRootPath, path) => {
            ...
            if (!((_a = configFileExistenceInfo.openFilesImpactedByConfigFile) === null || _a === void 0 ? void 0 : _a.has(path)))
                return;
            ...
            if (!newConfigFileNameForInfo)
                return;
            // Cache the existing config file info for this open file if not already done so
            if (!((_b = this.pendingOpenFileProjectUpdates) === null || _b === void 0 ? void 0 : _b.has(path))) {
                ((_c = this.pendingOpenFileProjectUpdates) !== null && _c !== void 0 ? _c : (this.pendingOpenFileProjectUpdates = new Map())).set(path, configFileForOpenFile);
            }
        });
    ...
    removeProject(project) {
        ...
        if (Debug.shouldAssert(AssertionLevel.Normal)) {
            this.filenameToScriptInfo.forEach(info => Debug.assert(!info.isAttached(project), "Found script Info still attached to project", () => `${project.projectName}: ScriptInfos still attached: ${JSON.stringify(arrayFrom(mapDefinedIterator(this.filenameToScriptInfo.values(), info => info.isAttached(project) ?
                {
    ...
    assignOrphanScriptInfoToInferredProject(info, projectRootPath) {
        ...
        const project = this.getOrCreateInferredProjectForProjectRootPathIfEnabled(info, projectRootPath) ||
            ...
            this.getOrCreateSingleInferredWithoutProjectRoot(info.isDynamic ?
                ...
                getDirectoryPath(isRootedDiskPath(info.fileName) ?
                    info.fileName :
                    getNormalizedAbsolutePath(info.fileName, projectRootPath ?
    ...
    closeOpenFile(info, skipAssignOrphanScriptInfosToInferredProject) {
        ...
        (_a = this.pendingOpenFileProjectUpdates) === null || _a === void 0 ? void 0 : _a.delete(info.path);
    ...
    configFileExists(configFileName, canonicalConfigFilePath, info) {
        ...
        if (this.openFiles.has(info.path) && (!isAncestorConfigFileInfo(info) || info.isForDefaultProject)) {
            ...
            if (configFileExistenceInfo)
                ((_a = configFileExistenceInfo.openFilesImpactedByConfigFile) !== null && _a !== void 0 ? _a : (configFileExistenceInfo.openFilesImpactedByConfigFile = new Set())).add(info.path);
            else
    ...
    createConfigFileWatcherForParsedConfig(configFileName, canonicalConfigFilePath, forProject) {
        ...
        if (!configFileExistenceInfo.watcher || configFileExistenceInfo.watcher === noopConfigFileWatcher) {
            configFileExistenceInfo.watcher = this.watchFactory.watchFile(configFileName, (_fileName, eventKind) => this.onConfigFileChanged(configFileName, canonicalConfigFilePath, eventKind), PollingInterval.High, this.getWatchOptionsFromProjectWatchOptions((_b = (_a = configFileExistenceInfo === null || configFileExistenceInfo === void 0 ? void 0 : configFileExistenceInfo.config) === null || _a === void 0 ? void 0 : _a.parsedCommandLine) === null || _b === void 0 ? void 0 : _b.watchOptions, getDirectoryPath(configFileName)), WatchType.ConfigFile, forProject);
        }
    ...
    stopWatchingConfigFilesForScriptInfo(info) {
        ...
        if (isOpen && !isRootOfInferredProject)
            return;
        this.forEachConfigFileLocation(info, canonicalConfigFilePath => {
            ...
            if (isOpen) {
                ...
                if (!((_b = configFileExistenceInfo.openFilesImpactedByConfigFile) === null || _b === void 0 ? void 0 : _b.delete(info.path)))
                    return;
            }
    ...
    startWatchingConfigFilesForInferredProjectRoot(info) {
        ...
        this.forEachConfigFileLocation(info, (canonicalConfigFilePath, configFileName) => {
            ...
            if (!configFileExistenceInfo) {
                ...
                configFileExistenceInfo.inferredProjectRoots = ((_a = configFileExistenceInfo.inferredProjectRoots) !== null && _a !== void 0 ? _a : 0) + 1;
            }
            ...
            ((_b = configFileExistenceInfo.openFilesImpactedByConfigFile) !== null && _b !== void 0 ? _b : (configFileExistenceInfo.openFilesImpactedByConfigFile = new Set())).add(info.path);
            // If there is no configured project for this config file, add the file watcher
            configFileExistenceInfo.watcher || (configFileExistenceInfo.watcher = canWatchDirectoryOrFilePath(getDirectoryPath(canonicalConfigFilePath)) ?
                this.watchFactory.watchFile(configFileName, (_filename, eventKind) => this.onConfigFileChanged(configFileName, canonicalConfigFilePath, eventKind), PollingInterval.High, this.hostConfiguration.watchOptions, WatchType.ConfigFileForInferredRoot) :
    ...
    forEachConfigFileLocation(info, action) {
        ...
        if (isAncestorConfigFileInfo(info)) {
            ...
            if (endsWith(info.fileName, "tsconfig.json"))
                searchTsconfig = false;
            else
                searchTsconfig = searchJsconfig = false;
        }
        do {
    ...
    findDefaultConfiguredProject(info) {
        var _a;
        return (_a = this.findDefaultConfiguredProjectWorker(info, ConfiguredProjectLoadKind.Find)) === null || _a === void 0 ? void 0 : _a.defaultProject;
    }
    /** @internal */
    findDefaultConfiguredProjectWorker(info, kind) {
        return info.isScriptOpen() ?
            this.tryFindDefaultConfiguredProjectForOpenScriptInfo(info, kind) :
            undefined;
    }
    ...
    setConfigFileNameForFileInCache(info, configFileName) {
        if (!this.openFiles.has(info.path))
            return; // Dont cache for closed script infos
        ...
        if (!isAncestorConfigFileInfo(info)) {
            ...
            if (!configFileForOpenFile || isString(configFileForOpenFile)) {
                ...
                this.configFileForOpenFiles.set(info.path, configFileForOpenFile = new Map().set(false, configFileForOpenFile));
            }
    ...
    findConfiguredProjectByProjectName(configFileName, allowDeferredClosed) {
        ...
        return allowDeferredClosed ? result : !(result === null || result === void 0 ? void 0 : result.deferredClose) ? result : undefined;
    }
    ...
    createExternalProject(projectFileName, files, options, typeAcquisition, excludedFiles) {
        ...
        const project = new ExternalProject(projectFileName, this, compilerOptions, 
        ...
        /*projectFilePath*/ undefined, watchOptionsAndErrors === null || watchOptionsAndErrors === void 0 ? void 0 : watchOptionsAndErrors.watchOptions);
    ...
    sendProjectTelemetry(project) {
        ...
        const projectOptions = isConfiguredProject(project) ? project.projectOptions : undefined;
        setProjectOptionsUsed(project);
        const data = {
    ...
    createConfiguredProject(configFileName, reason) {
        tracing === null || tracing === void 0 ? void 0 : tracing.instant(tracing.Phase.Session, "createConfiguredProject", { configFilePath: configFileName });
        const canonicalConfigFilePath = asNormalizedPath(this.toCanonicalFileName(configFileName));
        let configFileExistenceInfo = this.configFileExistenceInfoCache.get(canonicalConfigFilePath);
        ...
        const project = new ConfiguredProject(configFileName, canonicalConfigFilePath, this, configFileExistenceInfo.config.cachedDirectoryStructureHost, reason);
    ...
    loadConfiguredProject(project, reason) {
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Session, "loadConfiguredProject", { configFilePath: project.canonicalConfigFilePath });
        this.sendProjectLoadingStartEvent(project, reason);
        // Read updated contents from disk
        const configFilename = toNormalizedPath(project.getConfigFilePath());
        const configFileExistenceInfo = this.ensureParsedConfigUptoDate(configFilename, project.canonicalConfigFilePath, this.configFileExistenceInfoCache.get(project.canonicalConfigFilePath), project);
        const parsedCommandLine = configFileExistenceInfo.config.parsedCommandLine;
    ...
    ensureParsedConfigUptoDate(configFilename, canonicalConfigFilePath, configFileExistenceInfo, forProject) {
        ...
        const cachedDirectoryStructureHost = ((_a = configFileExistenceInfo.config) === null || _a === void 0 ? void 0 : _a.cachedDirectoryStructureHost) ||
        ...
        const parsedCommandLine = parseJsonSourceFileConfigFileContent(configFile, cachedDirectoryStructureHost, configDir, 
        /*existingOptions*/ undefined, configFilename, 
        /*resolutionStack*/ undefined, this.hostConfiguration.extraFileExtensions, this.extendedConfigCache);
        ...
        /*replacer*/ undefined, " ")}`);
        const oldCommandLine = (_b = configFileExistenceInfo.config) === null || _b === void 0 ? void 0 : _b.parsedCommandLine;
        if (!configFileExistenceInfo.config) {
        ...
        updateSharedExtendedConfigFileWatcher(canonicalConfigFilePath, parsedCommandLine.options, this.sharedExtendedConfigFileWatchers, (extendedConfigFileName, extendedConfigFilePath) => this.watchFactory.watchFile(extendedConfigFileName, () => {
            var _a;
            ...
            // Update projects
            let ensureProjectsForOpenFiles = false;
            (_a = this.sharedExtendedConfigFileWatchers.get(extendedConfigFilePath)) === null || _a === void 0 ? void 0 : _a.projects.forEach(canonicalPath => {
            ...
            if (ensureProjectsForOpenFiles)
                this.delayEnsureProjectForOpenFiles();
        }, PollingInterval.High, this.hostConfiguration.watchOptions, WatchType.ExtendedConfigFile, configFilename), fileName => this.toPath(fileName));
        return configFileExistenceInfo;
    }
    /** @internal */
    watchWildcards(configFileName, { exists, config }, forProject) {
        ...
        if (exists) {
            ...
            updateWatchingWildcardDirectories((_a = config).watchedDirectories || (_a.watchedDirectories = new Map()), config.parsedCommandLine.wildcardDirectories, 
    ...
    updateNonInferredProjectFiles(project, files, propertyReader) {
        ...
        for (const f of files) {
            ...
            if (!isDynamic && !project.fileExists(newRootFile)) {
                ...
                if (existingValue) {
                    if (((_a = existingValue.info) === null || _a === void 0 ? void 0 : _a.path) === path) {
                ...
                const scriptInfo = Debug.checkDefined(this.getOrCreateScriptInfoNotOpenedByClientForNormalizedPath(fileName, project.currentDirectory, scriptKind, hasMixedContent, project.directoryStructureHost, 
    ...
    reloadFileNamesOfConfiguredProject(project) {
        ...
        this.updateNonInferredProjectFiles(project, config.fileNames.concat(project.getExternalFiles(ProgramUpdateLevel.RootNamesAndUpdate)), fileNamePropertyReader);
        project.markAsDirty();
    ...
    reloadFileNamesOfParsedConfig(configFileName, config) {
        ...
        const fileNames = getFileNamesFromConfigSpecs(configFileSpecs, getDirectoryPath(configFileName), config.parsedCommandLine.options, config.cachedDirectoryStructureHost, this.hostConfiguration.extraFileExtensions);
    ...
    reloadConfiguredProjectOptimized(project, reason, reloadedProjects) {
        if (reloadedProjects.has(project))
            return;
    ...
    reloadConfiguredProjectClearingSemanticCache(project, reason, reloadedProjects) {
        if (reloadedProjects.get(project) === ConfiguredProjectLoadKind.Reload)
            return false;
    ...
    reloadConfiguredProject(project, reason) {
        ...
        updateWithTriggerFile(project, (_a = project.triggerFileForConfigFileDiag) !== null && _a !== void 0 ? _a : project.getConfigFilePath(), /*isReload*/ true);
    }
    ...
    sendConfigFileDiagEvent(project, triggerFile, force) {
        ...
        if (!force && diagnostics.length === ((_a = project.configDiagDiagnosticsReported) !== null && _a !== void 0 ? _a : 0))
            return false;
        project.configDiagDiagnosticsReported = diagnostics.length;
        this.eventHandler({
            eventName: ConfigFileDiagEvent,
            data: { configFileName: project.getConfigFilePath(), diagnostics, triggerFile: triggerFile !== null && triggerFile !== void 0 ? triggerFile : project.getConfigFilePath() },
        });
        return true;
    }
    ...
    getOrCreateSingleInferredProjectIfEnabled() {
        ...
        return this.createInferredProject(this.currentDirectory, 
    ...
    createInferredProject(currentDirectory, isSingleInferredProject, projectRootPath) {
        ...
        const project = new InferredProject(this, compilerOptions, watchOptionsAndErrors === null || watchOptionsAndErrors === void 0 ? void 0 : watchOptionsAndErrors.watchOptions, projectRootPath, currentDirectory, typeAcquisition);
        project.setProjectErrors(watchOptionsAndErrors === null || watchOptionsAndErrors === void 0 ? void 0 : watchOptionsAndErrors.errors);
    ...
    getOrCreateScriptInfoNotOpenedByClient(uncheckedFileName, currentDirectory, hostToQueryFileExistsOn, deferredDeleteOk) {
        return this.getOrCreateScriptInfoNotOpenedByClientForNormalizedPath(toNormalizedPath(uncheckedFileName), currentDirectory, 
        /*scriptKind*/ undefined, 
        /*hasMixedContent*/ undefined, hostToQueryFileExistsOn, deferredDeleteOk);
    }
    ...
    logErrorForScriptInfoNotFound(fileName) {
        const names = arrayFrom(mapDefinedIterator(this.filenameToScriptInfo.entries(), entry => entry[1].deferredDelete ? undefined : entry), ([path, scriptInfo]) => ({ path, fileName: scriptInfo.fileName }));
    ...
    watchClosedScriptInfo(info) {
        ...
        if (!info.isDynamicOrHasMixedContent() &&
            ...
            if (!this.host.getModifiedTime || indexOfNodeModules === -1) {
                info.fileWatcher = this.watchFactory.watchFile(info.fileName, (_fileName, eventKind) => this.onSourceFileChanged(info, eventKind), PollingInterval.Medium, this.hostConfiguration.watchOptions, WatchType.ClosedScriptInfo);
            }
            else {
    ...
    createNodeModulesWatcher(dir, dirPath) {
        let watcher = this.watchFactory.watchDirectory(dir, fileOrDirectory => {
            var _a;
            ...
            if (((_a = result.affectedModuleSpecifierCacheProjects) === null || _a === void 0 ? void 0 : _a.size) && (basename === "package.json" || basename === "node_modules")) {
                result.affectedModuleSpecifierCacheProjects.forEach(project => {
                    var _a;
                    (_a = project.getModuleSpecifierCache()) === null || _a === void 0 ? void 0 : _a.clear();
                });
            }
        ...
        }, WatchDirectoryFlags.Recursive, this.hostConfiguration.watchOptions, WatchType.NodeModules);
        const result = {
            refreshScriptInfoRefCount: 0,
            affectedModuleSpecifierCacheProjects: undefined,
            close: () => {
                var _a;
                if (watcher && !result.refreshScriptInfoRefCount && !((_a = result.affectedModuleSpecifierCacheProjects) === null || _a === void 0 ? void 0 : _a.size)) {
                    watcher.close();
                    watcher = undefined;
    ...
    watchPackageJsonsInNodeModules(dir, project) {
        ...
        Debug.assert(!((_a = watcher.affectedModuleSpecifierCacheProjects) === null || _a === void 0 ? void 0 : _a.has(project)));
        (watcher.affectedModuleSpecifierCacheProjects || (watcher.affectedModuleSpecifierCacheProjects = new Set())).add(project);
        return {
            close: () => {
                var _a;
                (_a = watcher.affectedModuleSpecifierCacheProjects) === null || _a === void 0 ? void 0 : _a.delete(project);
                watcher.close();
            },
        };
    }
    ...
    getOrCreateScriptInfoNotOpenedByClientForNormalizedPath(fileName, currentDirectory, scriptKind, hasMixedContent, hostToQueryFileExistsOn, deferredDeleteOk) {
        if (isRootedDiskPath(fileName) || isDynamicFileName(fileName)) {
            return this.getOrCreateScriptInfoWorker(fileName, currentDirectory, 
            /*openedByClient*/ false, 
            /*fileContent*/ undefined, scriptKind, !!hasMixedContent, hostToQueryFileExistsOn, deferredDeleteOk);
        }
    ...
    getDocumentPositionMapper(project, generatedFileName, sourceFileName) {
        ...
        const documentPositionMapper = getDocumentPositionMapper({ getCanonicalFileName: this.toCanonicalFileName, log: s => this.logger.info(s), getSourceFileLike: f => this.getSourceFileLike(f, projectName, declarationInfo) }, declarationInfo.fileName, declarationInfo.textStorage.getLineInfo(), readMapFile);
            ...
            if (!isString(sourceMapFileInfo)) {
                ...
                declarationInfo.sourceMapFilePath = {
                    watcher: this.addMissingSourceMapFile(project.currentDirectory === this.currentDirectory ?
                        sourceMapFileInfo :
                        getNormalizedAbsolutePath(sourceMapFileInfo, project.currentDirectory), declarationInfo.path),
    ...
    addMissingSourceMapFile(mapFileName, declarationInfoPath) {
        const fileWatcher = this.watchFactory.watchFile(mapFileName, () => {
        ...
        }, PollingInterval.High, this.hostConfiguration.watchOptions, WatchType.MissingSourceMapFile);
        return fileWatcher;
    }
    /** @internal */
    getSourceFileLike(fileName, projectNameOrProject, declarationInfo) {
        ...
        if (declarationInfo && isString(declarationInfo.sourceMapFilePath) && info !== declarationInfo) {
            ...
            if (sourceMapInfo) {
                ((_a = sourceMapInfo.sourceInfos) !== null && _a !== void 0 ? _a : (sourceMapInfo.sourceInfos = new Set())).add(info.path);
            }
        }
    ...
    setHostConfiguration(args) {
        ...
        if (args.file) {
            ...
            if (args.preferences) {
                const { lazyConfiguredProjectsFromExternalProject, includePackageJsonAutoImports, includeCompletionsForModuleExports, } = this.hostConfiguration.preferences;
                this.hostConfiguration.preferences = { ...this.hostConfiguration.preferences, ...args.preferences };
                if (lazyConfiguredProjectsFromExternalProject && !this.hostConfiguration.preferences.lazyConfiguredProjectsFromExternalProject) {
                    // Load configured projects for external projects that are pending reload
                    this.externalProjectToConfiguredProjectMap.forEach(projects => projects.forEach(project => {
            ...
            if (args.watchOptions) {
                const watchOptions = (_a = convertWatchOptions(args.watchOptions)) === null || _a === void 0 ? void 0 : _a.watchOptions;
    ...
    getWatchOptionsFromProjectWatchOptions(projectOptions, basePath) {
        const hostWatchOptions = !this.hostConfiguration.beforeSubstitution ? this.hostConfiguration.watchOptions :
            handleWatchOptionsConfigDirTemplateSubstitution(this.hostConfiguration.beforeSubstitution, basePath);
    ...
    sendSourceFileChange(inPath) {
        this.filenameToScriptInfo.forEach(info => {
            ...
            const eventKind = memoize(() => this.host.fileExists(info.fileName) ?
                info.deferredDelete ?
            ...
            if (inPath) {
                ...
                if (isScriptInfoWatchedFromNodeModules(info) || !info.path.startsWith(inPath))
                    return;
                // If we are sending delete event and its already deleted, ignore
                if (eventKind() === FileWatcherEventKind.Deleted && info.deferredDelete)
                    return;
    ...
    reloadProjects() {
        ...
        const reloadedConfiguredProjects = new Map();
        ...
        this.externalProjectToConfiguredProjectMap.forEach((projects, externalProjectName) => {
            ...
            projects.forEach(project => {
                if (this.getHostPreferences().lazyConfiguredProjectsFromExternalProject) {
                    ...
                    this.reloadConfiguredProjectClearingSemanticCache(project, reason, reloadedConfiguredProjects);
                }
            });
        });
        ...
        this.openFiles.forEach((_projectRootPath, path) => {
            ...
            if (find(info.containingProjects, isExternalProject))
                return;
            this.tryFindDefaultConfiguredProjectAndLoadAncestorsForOpenScriptInfo(info, ConfiguredProjectLoadKind.Reload, reloadedConfiguredProjects, delayReloadedConfiguredProjects);
        });
        ...
        this.ensureProjectForOpenFiles();
        // Cleanup
        this.cleanupProjectsAndScriptInfos(reloadedConfiguredProjects, new Set(this.openFiles.keys()), new Set(this.externalProjectToConfiguredProjectMap.keys()));
    ...
    ensureProjectForOpenFiles() {
        ...
        this.pendingOpenFileProjectUpdates = undefined;
        pendingOpenFileProjectUpdates === null || pendingOpenFileProjectUpdates === void 0 ? void 0 : pendingOpenFileProjectUpdates.forEach((_config, path) => this.tryFindDefaultConfiguredProjectAndLoadAncestorsForOpenScriptInfo(this.getScriptInfoForPath(path), ConfiguredProjectLoadKind.Create));
    ...
    getOriginalLocationEnsuringConfiguredProject(project, location) {
        ...
        const result = this.tryFindDefaultConfiguredProjectForOpenScriptInfoOrClosedFileInfo(originalFileInfo, ConfiguredProjectLoadKind.Create, updateProjectFoundUsingFind(configuredProject, ConfiguredProjectLoadKind.CreateOptimized), project => `Creating project referenced in solution ${project.projectName} to find possible configured project for original file: ${originalFileInfo.fileName}${location !== originalLocation ? " for location: " + location.fileName : ""}`);
        ...
        function addOriginalConfiguredProject(originalProject) {
            var _a;
            ((_a = project.originalConfiguredProjects) !== null && _a !== void 0 ? _a : (project.originalConfiguredProjects = new Set())).add(originalProject.canonicalConfigFilePath);
        }
    }
    /** @internal */
    fileExists(fileName) {
    ...
    getOrCreateOpenScriptInfo(fileName, fileContent, scriptKind, hasMixedContent, projectRootPath) {
        const info = this.getOrCreateScriptInfoWorker(fileName, projectRootPath ? this.getNormalizedAbsolutePath(projectRootPath) : this.currentDirectory, 
        /*openedByClient*/ true, fileContent, scriptKind, !!hasMixedContent, 
    ...
    assignProjectToOpenedScriptInfo(info) {
        ...
        if (!project && this.serverMode === LanguageServiceMode.Semantic) { // Checking semantic mode is an optimization
            const result = this.tryFindDefaultConfiguredProjectAndLoadAncestorsForOpenScriptInfo(info, ConfiguredProjectLoadKind.Create);
            if (result) {
        ...
        if (info.isOrphan()) {
            ...
            retainProjects === null || retainProjects === void 0 ? void 0 : retainProjects.forEach((kind, project) => {
                if (kind !== ConfiguredProjectLoadKind.CreateOptimized &&
     ...
     *  @internal
     */
    findCreateOrReloadConfiguredProject(configFileName, kind, 
    /** Used with ConfiguredProjectLoadKind.Create or ConfiguredProjectLoadKind.Reload for new projects or reload updates */
    reason, 
    /** Used with ConfiguredProjectLoadKind.Find to get deferredClosed projects as well */
    allowDeferredClosed, 
    ...
    projectForConfigFile) {
        let project = projectForConfigFile !== null && projectForConfigFile !== void 0 ? projectForConfigFile : this.findConfiguredProjectByProjectName(configFileName, allowDeferredClosed);
        let sentConfigFileDiag = false;
        let configFileExistenceInfo;
        switch (kind) {
            case ConfiguredProjectLoadKind.FindOptimized:
            ...
            case ConfiguredProjectLoadKind.CreateOptimized:
            case ConfiguredProjectLoadKind.Create:
                project !== null && project !== void 0 ? project : (project = this.createConfiguredProject(configFileName, reason));
                if (!delayLoad) {
                    // Ensure project is updated
                    ({ sentConfigFileDiag, configFileExistenceInfo } = updateProjectFoundUsingFind(project, kind, triggerFile));
                }
                break;
            case ConfiguredProjectLoadKind.ReloadOptimized:
                project !== null && project !== void 0 ? project : (project = this.createConfiguredProject(configFileName, reloadReason(reason)));
                project.projectService.reloadConfiguredProjectOptimized(project, reason, reloadedProjects);
                configFileExistenceInfo = configFileExistenceInfoForOptimizedLoading(project);
                if (configFileExistenceInfo)
                    break;
     ...
     */
    tryFindDefaultConfiguredProjectForOpenScriptInfo(info, kind, 
    /** Used with ConfiguredProjectLoadKind.Find to get deferredClosed projects as well */
    allowDeferredClosed, 
    /** Used with ConfiguredProjectLoadKind.Reload to check if this project was already reloaded */
    reloadedProjects) {
        ...
        const result = this.findCreateOrReloadConfiguredProject(configFileName, optimizedKind, fileOpenReason(info), allowDeferredClosed, info.fileName, reloadedProjects);
        // If the project for the configFileName does not exist, no result
        return result && this.tryFindDefaultConfiguredProjectForOpenScriptInfoOrClosedFileInfo(info, kind, result, project => `Creating project referenced in solution ${project.projectName} to find possible configured project for ${info.fileName} to open`, allowDeferredClosed, reloadedProjects);
    }
    isMatchedByConfig(configFileName, config, info) {
        // If info is root of the file
        if (config.fileNames.some(rootName => this.toPath(rootName) === info.path))
            return true;
        if (isSupportedSourceFileName(info.fileName, config.options, this.hostConfiguration.extraFileExtensions))
        ...
        if (validatedFilesSpec === null || validatedFilesSpec === void 0 ? void 0 : validatedFilesSpec.some(fileSpec => this.toPath(getNormalizedAbsolutePath(fileSpec, basePath)) === info.path))
            return true;
        if (!(validatedIncludeSpecs === null || validatedIncludeSpecs === void 0 ? void 0 : validatedIncludeSpecs.length))
            return false;
        // If this is excluded file ignore:
        if (matchesExcludeWorker(info.fileName, validatedExcludeSpecs, this.host.useCaseSensitiveFileNames, this.currentDirectory, basePath))
            return false;
        // If matched by include
        return validatedIncludeSpecs === null || validatedIncludeSpecs === void 0 ? void 0 : validatedIncludeSpecs.some(includeSpec => {
            const pattern = getPatternFromSpec(includeSpec, basePath, "files");
            return !!pattern && getRegexFromPattern(`(${pattern})$`, this.host.useCaseSensitiveFileNames).test(info.fileName);
        });
    }
    tryFindDefaultConfiguredProjectForOpenScriptInfoOrClosedFileInfo(info, kind, initialConfigResult, referencedProjectReason, 
    /** Used with ConfiguredProjectLoadKind.Find  to get deferredClosed projects as well */
    allowDeferredClosed, 
    /** Used with ConfiguredProjectLoadKind.Reload to check if this project was already reloaded */
    reloadedProjects) {
        ...
        return {
            defaultProject: defaultProject !== null && defaultProject !== void 0 ? defaultProject : possiblyDefault,
            tsconfigProject: tsconfigOfDefault !== null && tsconfigOfDefault !== void 0 ? tsconfigOfDefault : tsconfigOfPossiblyDefault,
            sentConfigDiag,
            seenProjects,
            seenConfigs,
        };
        function tryFindDefaultConfiguredProject(result) {
            var _a, _b;
            return (_b = (_a = isDefaultProjectOptimized(result, result.project)) !== null && _a !== void 0 ? _a : tryFindDefaultConfiguredProjectFromReferences(result.project)) !== null && _b !== void 0 ? _b : tryFindDefaultConfiguredProjectFromAncestor(result.project);
        }
        function isDefaultConfigFileExistenceInfo(configFileExistenceInfo, project, childConfigName, reason, tsconfigProject, canonicalConfigFilePath) {
            ...
            if (project) {
                ...
                if (seenConfigs === null || seenConfigs === void 0 ? void 0 : seenConfigs.has(canonicalConfigFilePath))
                    return;
                (seenConfigs !== null && seenConfigs !== void 0 ? seenConfigs : (seenConfigs = new Set())).add(canonicalConfigFilePath);
            }
            // If the file is listed in root files, then only we can use this project as default project
            if (!tsconfigProject.projectService.isMatchedByConfig(childConfigName, configFileExistenceInfo.config.parsedCommandLine, info)) {
                if (tsconfigProject.languageServiceEnabled) {
                    // Ensure we are watching the parsedCommandLine
                    tsconfigProject.projectService.watchWildcards(childConfigName, configFileExistenceInfo, tsconfigProject);
                }
                return;
            }
            // Ensure the project is uptodate and created since the file may belong to this project
            const result = project ?
                updateProjectFoundUsingFind(project, kind, info.fileName, reason, reloadedProjects) :
                tsconfigProject.projectService.findCreateOrReloadConfiguredProject(childConfigName, kind, reason, allowDeferredClosed, info.fileName, reloadedProjects);
            if (!result) {
            ...
            if (result.sentConfigFileDiag)
                sentConfigDiag.add(result.project);
        ...
        function isDefaultProject(project, tsconfigProject) {
            ...
            if (seenProjects.get(project) === kind)
                return;
        ...
        function isDefaultProjectOptimized(result, tsconfigProject) {
            ...
            return result.configFileExistenceInfo ?
                isDefaultConfigFileExistenceInfo(result.configFileExistenceInfo, result.project, toNormalizedPath(result.project.getConfigFilePath()), result.reason, result.project, result.project.canonicalConfigFilePath) :
        ...
        function tryFindDefaultConfiguredProjectFromReferences(project) {
            ...
            return project.parsedCommandLine &&
                forEachResolvedProjectReferenceProjectLoad(project, project.parsedCommandLine, isDefaultConfigFileExistenceInfo, optimizedKind, referencedProjectReason(project), allowDeferredClosed, reloadedProjects);
        }
        function tryFindDefaultConfiguredProjectFromAncestor(project) {
            ...
            info, project, tryFindDefaultConfiguredProject, optimizedKind, `Creating possible configured project for ${info.fileName} to open`, allowDeferredClosed, reloadedProjects, 
            /*searchOnlyPotentialSolution*/ false) : undefined;
        }
    }
    tryFindDefaultConfiguredProjectAndLoadAncestorsForOpenScriptInfo(info, kind, reloadedProjects, delayReloadedConfiguredProjects) {
        ...
        const result = this.tryFindDefaultConfiguredProjectForOpenScriptInfo(info, kind, allowDeferredClosed, reloadedProjects);
        if (!result)
            return;
        const { defaultProject, tsconfigProject, seenProjects } = result;
        if (defaultProject) {
            // Create ancestor tree for findAllRefs (dont load them right away)
            forEachAncestorProjectLoad(info, tsconfigProject, ancestor => {
                seenProjects.set(ancestor.project, kind);
            }, kind, `Creating project possibly referencing default composite project ${defaultProject.getProjectName()} of open file ${info.fileName}`, allowDeferredClosed, reloadedProjects, 
            /*searchOnlyPotentialSolution*/ true, delayReloadedConfiguredProjects);
        }
        return result;
    }
    /** @internal */
    loadAncestorProjectTree(forProjects) {
        forProjects !== null && forProjects !== void 0 ? forProjects : (forProjects = new Set(mapDefinedIterator(this.configuredProjects.entries(), ([key, project]) => !project.initialLoadPending ? key : undefined)));
    ...
    ensureProjectChildren(project, forProjects, seenProjects) {
        ...
        if (project.getCompilerOptions().disableReferencedProjectLoad)
            return;
        const children = (_a = project.getCurrentProgram()) === null || _a === void 0 ? void 0 : _a.getResolvedProjectReferences();
        if (!children)
            return;
        for (const child of children) {
            ...
            const childProject = (_b = this.findConfiguredProjectByProjectName(configFileName)) !== null && _b !== void 0 ? _b : this.createConfiguredProject(configFileName, `Creating project referenced by : ${project.projectName} as it references project ${referencedProject.sourceFile.fileName}`);
    ...
    cleanupConfiguredProjects(toRetainConfiguredProjects, externalProjectsRetainingConfiguredProjects, openFilesWithRetainedConfiguredProject) {
        ...
        this.getOrphanConfiguredProjects(toRetainConfiguredProjects, openFilesWithRetainedConfiguredProject, externalProjectsRetainingConfiguredProjects).forEach(project => this.removeProject(project));
    }
    cleanupProjectsAndScriptInfos(toRetainConfiguredProjects, openFilesWithRetainedConfiguredProject, externalProjectsRetainingConfiguredProjects) {
        ...
        this.cleanupConfiguredProjects(toRetainConfiguredProjects, externalProjectsRetainingConfiguredProjects, openFilesWithRetainedConfiguredProject);
    ...
    tryInvokeWildCardDirectories(info) {
        ...
        this.configFileExistenceInfoCache.forEach((configFileExistenceInfo, config) => {
            var _a, _b;
            if (!((_a = configFileExistenceInfo.config) === null || _a === void 0 ? void 0 : _a.parsedCommandLine) ||
                contains(configFileExistenceInfo.config.parsedCommandLine.fileNames, info.fileName, !this.host.useCaseSensitiveFileNames ? equateStringsCaseInsensitive : equateStringsCaseSensitive)) {
                return;
            }
            (_b = configFileExistenceInfo.config.watchedDirectories) === null || _b === void 0 ? void 0 : _b.forEach((watcher, directory) => {
                if (containsPath(directory, info.fileName, !this.host.useCaseSensitiveFileNames)) {
                    ...
                    this.onWildCardDirectoryWatcherInvoke(directory, config, configFileExistenceInfo.config, watcher.watcher, info.fileName);
                }
            });
        });
    }
    openClientFileWithNormalizedPath(fileName, fileContent, scriptKind, hasMixedContent, projectRootPath) {
        ...
        if (!existing && info && !info.isDynamic)
            this.tryInvokeWildCardDirectories(info);
        const { retainProjects, ...result } = this.assignProjectToOpenedScriptInfo(info);
        this.cleanupProjectsAndScriptInfos(retainProjects, new Set([info.path]), 
    ...
    getOrphanConfiguredProjects(toRetainConfiguredProjects, openFilesWithRetainedConfiguredProject, externalProjectsRetainingConfiguredProjects) {
        ...
        toRetainConfiguredProjects === null || toRetainConfiguredProjects === void 0 ? void 0 : toRetainConfiguredProjects.forEach((_, project) => retainConfiguredProject(project));
        ...
        this.externalProjectToConfiguredProjectMap.forEach((projects, externalProjectName) => {
            if (!(externalProjectsRetainingConfiguredProjects === null || externalProjectsRetainingConfiguredProjects === void 0 ? void 0 : externalProjectsRetainingConfiguredProjects.has(externalProjectName))) {
        ...
        forEachEntry(this.openFiles, (_projectRootPath, path) => {
            if (openFilesWithRetainedConfiguredProject === null || openFilesWithRetainedConfiguredProject === void 0 ? void 0 : openFilesWithRetainedConfiguredProject.has(path))
                return;
            const info = this.getScriptInfoForPath(path);
            ...
            if (result === null || result === void 0 ? void 0 : result.defaultProject) {
                result === null || result === void 0 ? void 0 : result.seenProjects.forEach((_, project) => retainConfiguredProject(project));
                // Everything needs to be retained, fast path to skip all the work
                if (!toRemoveConfiguredProjects.size)
                    return toRemoveConfiguredProjects;
            }
        ...
        function isPendingUpdate(project) {
            ...
            return (project.deferredClose ||
                ...
                !!((_b = (_a = project.projectService.configFileExistenceInfoCache.get(project.canonicalConfigFilePath)) === null || _a === void 0 ? void 0 : _a.openFilesImpactedByConfigFile) === null || _b === void 0 ? void 0 : _b.size);
        }
        ...
        this.filenameToScriptInfo.forEach(info => {
            ...
            if (!info.isScriptOpen() &&
                ...
                if (isString(info.sourceMapFilePath)) {
                    ...
                    sourceInfos = sourceMapInfo === null || sourceMapInfo === void 0 ? void 0 : sourceMapInfo.sourceInfos;
                }
                else {
            ...
            if (info.sourceMapFilePath) {
                ...
                if (isString(info.sourceMapFilePath)) {
                    ...
                    const sourceMapInfo = this.filenameToScriptInfo.get(info.sourceMapFilePath);
                    if (sourceMapInfo === null || sourceMapInfo === void 0 ? void 0 : sourceMapInfo.deferredDelete) {
    ...
    applyChangesInOpenFiles(openFiles, changedFiles, closedFiles) {
        ...
        if (openFiles) {
            for (const file of openFiles) {
                (existingOpenScriptInfos !== null && existingOpenScriptInfos !== void 0 ? existingOpenScriptInfos : (existingOpenScriptInfos = [])).push(this.getScriptInfoForPath(normalizedPathToPath(toNormalizedPath(file.fileName), file.projectRootPath ? this.getNormalizedAbsolutePath(file.projectRootPath) : this.currentDirectory, this.toCanonicalFileName)));
                // Create script infos so we have the new content for all the open files before we do any updates to projects
                const info = this.getOrCreateOpenScriptInfo(toNormalizedPath(file.fileName), file.content, tryConvertScriptKindName(file.scriptKind), file.hasMixedContent, file.projectRootPath ? toNormalizedPath(file.projectRootPath) : undefined);
        ...
        let retainProjects;
        forEach(existingOpenScriptInfos, (existing, index) => !existing && openScriptInfos[index] && !openScriptInfos[index].isDynamic ?
            this.tryInvokeWildCardDirectories(openScriptInfos[index]) :
            undefined);
        openScriptInfos === null || openScriptInfos === void 0 ? void 0 : openScriptInfos.forEach(info => {
            var _a;
            return (_a = this.assignProjectToOpenedScriptInfo(info).retainProjects) === null || _a === void 0 ? void 0 : _a.forEach((kind, p) => (retainProjects !== null && retainProjects !== void 0 ? retainProjects : (retainProjects = new Map())).set(p, kind));
        });
        ...
        if (openScriptInfos) {
            // Cleanup projects
            this.cleanupProjectsAndScriptInfos(retainProjects, new Set(openScriptInfos.map(info => info.path)), 
            /*externalProjectsRetainingConfiguredProjects*/ undefined);
            // Telemetry
    ...
    applySafeList(proj) {
        ...
        return (_a = result === null || result === void 0 ? void 0 : result.excludedFiles) !== null && _a !== void 0 ? _a : [];
    }
        ...
        for (let i = 0; i < rootFiles.length; i++) {
            if (excludeRegexes.some(re => re.test(normalizedNames[i]))) {
                ...
                if (/^.+[.-]min\.js$/.test(normalizedNames[i])) {
                    ...
                    filesToKeep === null || filesToKeep === void 0 ? void 0 : filesToKeep.push(rootFiles[i]);
                }
            }
        }
    ...
    openExternalProject(proj, cleanupAfter) {
        ...
        for (const file of proj.rootFiles) {
            ...
            if (getBaseConfigFileName(normalized)) {
                if (this.serverMode === LanguageServiceMode.Semantic && this.host.fileExists(normalized)) {
                    ...
                    (configuredProjects !== null && configuredProjects !== void 0 ? configuredProjects : (configuredProjects = new Set())).add(project);
        ...
        if (configuredProjects) {
            ...
            const excludedFiles = (_a = excludeResult === null || excludeResult === void 0 ? void 0 : excludeResult.excludedFiles) !== null && _a !== void 0 ? _a : [];
            rootFiles = (_b = excludeResult === null || excludeResult === void 0 ? void 0 : excludeResult.rootFiles) !== null && _b !== void 0 ? _b : rootFiles;
            if (existingExternalProject) {
                ...
                existingExternalProject.setProjectErrors(watchOptionsAndErrors === null || watchOptionsAndErrors === void 0 ? void 0 : watchOptionsAndErrors.errors);
                // external project already exists and not config files were added - update the project and return;
                // The graph update here isnt postponed since any file open operation needs all updated external projects
                this.updateRootAndOptionsOfNonInferredProject(existingExternalProject, rootFiles, externalFilePropertyReader, compilerOptions, typeAcquisition, proj.options.compileOnSave, watchOptionsAndErrors === null || watchOptionsAndErrors === void 0 ? void 0 : watchOptionsAndErrors.watchOptions);
        ...
        if (cleanupAfter) {
            this.cleanupConfiguredProjects(configuredProjects, new Set([proj.projectFileName]));
            this.printProjects();
        }
    }
    ...
    requestEnablePlugin(project, pluginConfigEntry, searchPaths) {
        ...
        if (this.host.importPlugin) {
            const importPromise = Project.importServicePluginAsync(pluginConfigEntry, searchPaths, this.host, s => this.logger.info(s));
            (_a = this.pendingPluginEnablements) !== null && _a !== void 0 ? _a : (this.pendingPluginEnablements = new Map());
        ...
        this.endEnablePlugin(project, Project.importServicePluginSync(pluginConfigEntry, searchPaths, this.host, s => this.logger.info(s)));
    }
    /**
    ...
    endEnablePlugin(project, { pluginConfigEntry, resolvedModule, errorLogs }) {
        var _a;
        if (resolvedModule) {
            const configurationOverride = (_a = this.currentPluginConfigOverrides) === null || _a === void 0 ? void 0 : _a.get(pluginConfigEntry.name);
    ...
    getPackageJsonsVisibleToFile(fileName, project, rootDir) {
        ...
        forEachAncestorDirectoryStoppingAtGlobalCache(project, getDirectoryPath(fileName), processDirectory);
        return result;
    }
    /** @internal */
    getNearestAncestorDirectoryWithPackageJson(fileName, project) {
        return forEachAncestorDirectoryStoppingAtGlobalCache(project, fileName, directory => {
    ...
    watchPackageJsonFile(file, path, project) {
        ...
        let result = ((_a = this.packageJsonFilesMap) !== null && _a !== void 0 ? _a : (this.packageJsonFilesMap = new Map())).get(path);
        if (!result) {
            ...
            }, PollingInterval.Low, this.hostConfiguration.watchOptions, WatchType.PackageJson);
            result = {
                projects: new Set(),
                close: () => {
                    var _a;
                    ...
                    watcher.close();
                    watcher = undefined;
                    (_a = this.packageJsonFilesMap) === null || _a === void 0 ? void 0 : _a.delete(path);
        ...
        ((_b = project.packageJsonWatches) !== null && _b !== void 0 ? _b : (project.packageJsonWatches = new Set())).add(result);
    }
    onPackageJsonChange(result) {
        result.projects.forEach(project => { var _a, _b; return (_b = (_a = project).onPackageJsonChange) === null || _b === void 0 ? void 0 : _b.call(_a); });
    }
    /** @internal */
...
/** @internal */
export function isConfigFile(config) {

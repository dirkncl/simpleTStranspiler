...
(function (CompletionSource) {
    ...
    CompletionSource["SwitchCases"] = "SwitchCases/";
    /** Completions for an Object literal expression */
    CompletionSource["ObjectLiteralMemberWithComma"] = "ObjectLiteralMemberWithComma/";
})(CompletionSource || (CompletionSource = {}));
// dprint-ignore
/** @internal */
export var SymbolOriginInfoKind;
(function (SymbolOriginInfoKind) {
    ...
    SymbolOriginInfoKind[SymbolOriginInfoKind["Ignore"] = 256] = "Ignore";
    SymbolOriginInfoKind[SymbolOriginInfoKind["ComputedPropertyName"] = 512] = "ComputedPropertyName";
...
function originIsIgnore(origin) {
    return !!(origin && origin.kind & 256 /* SymbolOriginInfoKind.Ignore */);
}
function originIsComputedPropertyName(origin) {
    return !!(origin && origin.kind & 512 /* SymbolOriginInfoKind.ComputedPropertyName */);
}
// dprint-ignore
...
function resolvingModuleSpecifiers(logPrefix, host, resolver, program, position, preferences, isForImportStatementCompletion, isValidTypeOnlyUseSite, cb) {
    ...
    const needsFullResolution = isForImportStatementCompletion
        ...
        || ((_a = preferences.autoImportSpecifierExcludeRegexes) === null || _a === void 0 ? void 0 : _a.length);
    ...
    (_b = host.log) === null || _b === void 0 ? void 0 : _b.call(host, `${logPrefix}: resolved ${resolvedCount} module specifiers, plus ${ambientCount} ambient and ${resolvedFromCacheCount} from cache${hitRateMessage}`);
    (_c = host.log) === null || _c === void 0 ? void 0 : _c.call(host, `${logPrefix}: response is ${skippedAny ? "incomplete" : "complete"}`);
    (_d = host.log) === null || _d === void 0 ? void 0 : _d.call(host, `${logPrefix}: ${timestamp() - start}`);
    return result;
...
/** @internal */
export function getDefaultCommitCharacters(isNewIdentifierLocation) {
...
/** @internal */
export function getCompletionsAtPosition(host, program, log, sourceFile, position, preferences, triggerCharacter, completionKind, cancellationToken, formatContext, includeSymbol = false) {
    var _a;
    ...
    const incompleteCompletionsCache = preferences.allowIncompleteCompletions ? (_a = host.getIncompleteCompletionsCache) === null || _a === void 0 ? void 0 : _a.call(host) : undefined;
    ...
    switch (completionData.kind) {
        ...
        case 1 /* CompletionDataKind.JsDocTagName */:
            ...
            return jsdocCompletionInfo([
                ...
                ...getJSDocParameterCompletions(sourceFile, position, checker, compilerOptions, preferences, 
                /*tagNameOnly*/ true),
            ]);
...
function compareCompletionEntries(entryInArray, entryToInsert) {
    ...
    if (result === Comparison.EqualTo && ((_a = entryInArray.data) === null || _a === void 0 ? void 0 : _a.moduleSpecifier) && ((_b = entryToInsert.data) === null || _b === void 0 ? void 0 : _b.moduleSpecifier)) {
...
function continuePreviousIncompleteResponse(cache, file, location, program, host, preferences, cancellationToken, position) {
    ...
    const newEntries = resolvingModuleSpecifiers("continuePreviousIncompleteResponse", host, codefix.createImportSpecifierResolver(file, program, host, preferences), program, location.getStart(), preferences, 
    /*isForImportStatementCompletion*/ false, isValidTypeOnlyAliasUseSite(location), context => {
        const entries = mapDefined(previousResponse.entries, entry => {
            ...
            if (!result || result === "failed") {
                (_a = host.log) === null || _a === void 0 ? void 0 : _a.call(host, `Unexpected failure resolving auto import for '${entry.name}' from '${entry.source}'`);
                return undefined;
            }
            const newOrigin = Object.assign(Object.assign({}, origin), { kind: 32 /* SymbolOriginInfoKind.ResolvedExport */, moduleSpecifier: result.moduleSpecifier });
...
function getJSDocParameterCompletions(sourceFile, position, checker, options, preferences, tagNameOnly) {
    ...
    return mapDefined(func.parameters, param => {
        ...
        if (isIdentifier(param.name)) { // Named parameter
            ...
            let displayText = getJSDocParamAnnotation(paramName, param.initializer, param.dotDotDotToken, isJs, 
            /*isObject*/ false, 
            /*isSnippet*/ false, checker, options, preferences);
            let snippetText = isSnippet
                ? getJSDocParamAnnotation(paramName, param.initializer, param.dotDotDotToken, isJs, 
                /*isObject*/ false, 
                /*isSnippet*/ true, checker, options, preferences, tabstopCounter)
                : undefined;
            ...
            const displayTextResult = generateJSDocParamTagsForDestructuring(paramPath, param.name, param.initializer, param.dotDotDotToken, isJs, 
            /*isSnippet*/ false, checker, options, preferences);
            const snippetTextResult = isSnippet
                ? generateJSDocParamTagsForDestructuring(paramPath, param.name, param.initializer, param.dotDotDotToken, isJs, 
                /*isSnippet*/ true, checker, options, preferences)
                : undefined;
            let displayText = displayTextResult.join(getNewLineCharacter(options) + "* ");
            let snippetText = snippetTextResult === null || snippetTextResult === void 0 ? void 0 : snippetTextResult.join(getNewLineCharacter(options) + "* ");
            if (tagNameOnly) { // Remove `@`
                displayText = displayText.slice(1);
                if (snippetText)
                    snippetText = snippetText.slice(1);
...
function generateJSDocParamTagsForDestructuring(path, pattern, initializer, dotDotDotToken, isJs, isSnippet, checker, options, preferences) {
    if (!isJs) {
        return [
            getJSDocParamAnnotation(path, initializer, dotDotDotToken, isJs, 
            /*isObject*/ false, isSnippet, checker, options, preferences, { tabstop: 1 }),
        ];
    }
    return patternWorker(path, pattern, initializer, dotDotDotToken, { tabstop: 1 });
    function patternWorker(path, pattern, initializer, dotDotDotToken, counter) {
        if (isObjectBindingPattern(pattern) && !dotDotDotToken) {
            const oldTabstop = counter.tabstop;
            const childCounter = { tabstop: oldTabstop };
            const rootParam = getJSDocParamAnnotation(path, initializer, dotDotDotToken, isJs, 
            /*isObject*/ true, isSnippet, checker, options, preferences, childCounter);
            let childTags = [];
    ...
    function elementWorker(path, element, counter) {
        if ((!element.propertyName && isIdentifier(element.name)) || isIdentifier(element.name)) { // `{ b }` or `{ b: newB }`
            ...
            return [
                getJSDocParamAnnotation(paramName, element.initializer, element.dotDotDotToken, isJs, 
                /*isObject*/ false, isSnippet, checker, options, preferences, counter),
            ];
        }
...
function keywordFiltersFromSyntaxKind(keywordCompletion) {
    switch (keywordCompletion) {
        case SyntaxKind.TypeKeyword:
            return 8 /* KeywordCompletionFilters.TypeKeyword */;
        default:
...
function getOptionalReplacementSpan(location) {
    ...
    return (location === null || location === void 0 ? void 0 : location.kind) === SyntaxKind.Identifier ? createTextSpanFromNode(location) : undefined;
}
function completionInfoFromData(sourceFile, host, program, compilerOptions, log, completionData, preferences, formatContext, position, includeSymbol) {
        ...
        symbols.forEach((symbol, i) => {
            if (symbol.valueDeclaration && isEnumMember(symbol.valueDeclaration)) {
                ...
                if (value !== undefined && tracker.hasValue(value)) {
                    symbolToOriginInfoMap[i] = { kind: 256 /* SymbolOriginInfoKind.Ignore */ };
                }
            }
        });
    }
    ...
    return {
        ...
        entries,
        defaultCommitCharacters: defaultCommitCharacters !== null && defaultCommitCharacters !== void 0 ? defaultCommitCharacters : getDefaultCommitCharacters(isNewIdentifierLocation),
    };
}
...
function getExhaustiveCaseSnippets(caseBlock, sourceFile, preferences, options, host, program, formatContext) {
    ...
    if (switchType && switchType.isUnion() && every(switchType.types, type => type.isLiteral())) {
        ...
        const newLineChar = getNewLineOrDefaultFromHost(host, formatContext === null || formatContext === void 0 ? void 0 : formatContext.options);
        ...
        const printNode = formatContext
            ? (node) => printer.printAndFormatNode(EmitHint.Unspecified, node, sourceFile, formatContext)
            : (node) => printer.printNode(EmitHint.Unspecified, node, sourceFile);
...
function entityNameToExpression(entityName, languageVersion, quotePreference) {
    ...
    if (canUsePropertyAccess(unescapedName, languageVersion)) {
        ...
        return factory.createElementAccessExpression(entityNameToExpression(entityName.left, languageVersion, quotePreference), factory.createStringLiteral(unescapedName, quotePreference === QuotePreference.Single));
    }
}
...
function createCompletionEntry(symbol, sortText, replacementToken, contextToken, location, position, sourceFile, host, program, name, needsConvertPropertyAccess, origin, recommendedCompletion, propertyAccessToConvert, isJsxInitializer, importStatementCompletion, useSemicolons, options, preferences, completionKind, formatContext, isJsxIdentifierExpected, isRightOfOpenTag, includeSymbol) {
    ...
    if (completionKind === 0 /* CompletionKind.ObjectPropertyDeclaration */ && contextToken && ((_a = findPrecedingToken(contextToken.pos, sourceFile, contextToken)) === null || _a === void 0 ? void 0 : _a.kind) !== SyntaxKind.CommaToken) {
        if (isMethodDeclaration(contextToken.parent.parent) ||
            ...
            ((_b = findAncestor(contextToken.parent, isPropertyAssignment)) === null || _b === void 0 ? void 0 : _b.getLastToken(sourceFile)) === contextToken ||
    ...
    if (preferences.includeCompletionsWithClassMemberSnippets &&
        ...
        if (memberCompletionEntry) {
            ...
            if ((importAdder === null || importAdder === void 0 ? void 0 : importAdder.hasFixes()) || memberCompletionEntry.eraseRange) {
                hasAction = true;
    ...
    if (isJsxIdentifierExpected && !isRightOfOpenTag
        ...
        if (preferences.jsxAttributeCompletionStyle === "auto"
            ...
            && !(type.flags & TypeFlags.Union && find(type.types, type => !!(type.flags & TypeFlags.BooleanLike)))) {
            if (type.flags & TypeFlags.StringLike || (type.flags & TypeFlags.Union && every(type.types, type => !!(type.flags & (TypeFlags.StringLike | TypeFlags.Undefined) || isStringAndEmptyAnonymousObjectIntersection(type))))) {
    ...
    return Object.assign({ name,
        kind, kindModifiers: SymbolDisplay.getSymbolModifiers(typeChecker, symbol), sortText,
        source, hasAction: hasAction ? true : undefined, isRecommended: isRecommendedCompletionMatch(symbol, recommendedCompletion, typeChecker) || undefined, insertText,
        filterText,
        replacementSpan,
        sourceDisplay,
        labelDetails,
        isSnippet, isPackageJsonImport: originIsPackageJsonImport(origin) || undefined, isImportStatementCompletion: !!importStatementCompletion || undefined, data,
        commitCharacters }, includeSymbol ? { symbol } : undefined);
}
...
function getEntryForMemberCompletion(host, program, options, preferences, name, symbol, location, position, contextToken, formatContext) {
    ...
    const printer = createSnippetPrinter({
        ...
        newLine: getNewLineKind(getNewLineOrDefaultFromHost(host, formatContext === null || formatContext === void 0 ? void 0 : formatContext.options)),
    });
    const importAdder = codefix.createImportAdder(sourceFile, program, preferences, host);
    ...
    let completionNodes = [];
    codefix.addNewNodeForMemberSymbol(symbol, classLikeDeclaration, sourceFile, { program, host }, preferences, importAdder, 
    ...
    }, body, codefix.PreserveOptionalFlags.Property, !!isAbstract);
    if (completionNodes.length) {
        ...
        if (presentDecorators === null || presentDecorators === void 0 ? void 0 : presentDecorators.length) {
...
function createObjectLiteralMethod(symbol, enclosingDeclaration, sourceFile, program, host, preferences) {
    ...
    switch (declaration.kind) {
        ...
        case SyntaxKind.MethodDeclaration: {
            let effectiveType = type.flags & TypeFlags.Union && type.types.length < 10
                ? checker.getUnionType(type.types, UnionReduction.Subtype)
                : type;
            if (effectiveType.flags & TypeFlags.Union) {
                // Only offer the completion if there's a single function type component.
                const functionTypes = filter(effectiveType.types, type => checker.getSignaturesOfType(type, SignatureKind.Call).length > 0);
...
function createSnippetPrinter(printerOptions) {
    ...
    function printAndFormatSnippetList(format, list, sourceFile, formatContext) {
        ...
        const changes = flatMap(list, node => {
            ...
            return formatting.formatNodeGivenIndentation(nodeWithPos, syntheticFile, sourceFile.languageVariant, 
            /* indentation */ 0, 
            /* delta */ 0, Object.assign(Object.assign({}, formatContext), { options: formatOptions }));
        });
    ...
    function printAndFormatNode(hint, node, sourceFile, formatContext) {
        ...
        const formatOptions = getFormatCodeSettingsForWriting(formatContext, sourceFile);
        const nodeWithPos = textChanges.assignPositionsToNode(node);
        const changes = formatting.formatNodeGivenIndentation(nodeWithPos, syntheticFile, sourceFile.languageVariant, 
        /* indentation */ 0, 
        /* delta */ 0, Object.assign(Object.assign({}, formatContext), { options: formatOptions }));
        const allChanges = escapes
            ? toSorted(concatenate(changes, escapes), (a, b) => compareTextSpans(a.span, b.span))
...
export function getCompletionEntriesFromSymbols(symbols, entries, replacementToken, contextToken, location, position, sourceFile, host, program, target, log, kind, preferences, compilerOptions, formatContext, isTypeOnlyLocation, propertyAccessToConvert, jsxIdentifierExpected, isJsxInitializer, importStatementCompletion, recommendedCompletion, symbolToOriginInfoMap, symbolToSortTextMap, isJsxIdentifierExpected, isRightOfOpenTag, includeSymbol = false) {
    ...
    for (let i = 0; i < symbols.length; i++) {
        ...
        const origin = symbolToOriginInfoMap === null || symbolToOriginInfoMap === void 0 ? void 0 : symbolToOriginInfoMap[i];
        ...
        const originalSortText = (_a = symbolToSortTextMap === null || symbolToSortTextMap === void 0 ? void 0 : symbolToSortTextMap[getSymbolId(symbol)]) !== null && _a !== void 0 ? _a : SortText.LocationPriority;
    ...
    function shouldIncludeSymbol(symbol, symbolToSortTextMap) {
        ...
        if (!isSourceFile(location)) {
            ...
            const symbolDeclaration = (_a = symbol.valueDeclaration) !== null && _a !== void 0 ? _a : (_b = symbol.declarations) === null || _b === void 0 ? void 0 : _b[0];
            if (closestSymbolDeclaration && symbolDeclaration) {
                if (isParameter(closestSymbolDeclaration) && isParameter(symbolDeclaration)) {
                    ...
                    if (closestSymbolDeclaration === symbolDeclaration && (contextToken === null || contextToken === void 0 ? void 0 : contextToken.kind) === SyntaxKind.ExtendsKeyword) {
    ...
    function symbolAppearsToBeTypeOnly(symbol) {
        ...
        return !(flags & SymbolFlags.Value) && (!isInJSFile((_a = symbol.declarations) === null || _a === void 0 ? void 0 : _a[0]) || !!(flags & SymbolFlags.Type));
    }
}
...
function getSymbolCompletionFromEntryId(program, log, sourceFile, position, entryId, host, preferences) {
    ...
    return firstDefined(symbols, (symbol, index) => {
        ...
        return info && info.name === entryId.name && (entryId.source === CompletionSource.ClassMemberSnippet && symbol.flags & SymbolFlags.ClassMember
            ...
            ? { type: "symbol", symbol, location, origin, contextToken, previousToken, isJsxInitializer, isTypeOnlyLocation }
            : undefined;
    }) || { type: "none" };
}
/** @internal */
export function getCompletionEntryDetails(program, log, sourceFile, position, entryId, host, formatContext, preferences, cancellationToken) {
    const typeChecker = program.getTypeChecker();
    const compilerOptions = program.getCompilerOptions();
    ...
    switch (symbolCompletion.type) {
        ...
        case "cases": {
            const snippets = getExhaustiveCaseSnippets(contextToken.parent, sourceFile, preferences, program.getCompilerOptions(), host, program, 
            /*formatContext*/ undefined);
            if (snippets === null || snippets === void 0 ? void 0 : snippets.importAdder.hasFixes()) {
...
/** @internal */
export function createCompletionDetailsForSymbol(symbol, name, checker, sourceFile, location, cancellationToken, codeActions, sourceDisplay) {
...
/** @internal */
export function createCompletionDetails(name, kindModifiers, kind, displayParts, documentation, tags, codeActions, source) {
...
function getCompletionEntryCodeActionsAndSourceDisplay(name, location, contextToken, origin, symbol, program, host, compilerOptions, sourceFile, position, previousToken, formatContext, preferences, data, source, cancellationToken) {
    ...
    if (source === CompletionSource.ClassMemberSnippet) {
        ...
        if ((importAdder === null || importAdder === void 0 ? void 0 : importAdder.hasFixes()) || eraseRange) {
            const changes = textChanges.ChangeTracker.with({ host, formatContext, preferences }, tracker => {
                if (importAdder) {
            ...
            return {
                ...
                codeActions: [{
                        changes,
                        description: (importAdder === null || importAdder === void 0 ? void 0 : importAdder.hasFixes()) ?
    ...
    if (originIsTypeOnlyAlias(origin)) {
        const codeAction = codefix.getPromoteTypeOnlyCompletionAction(sourceFile, origin.declaration.name, program, host, formatContext, preferences);
    ...
    if (source === CompletionSource.ObjectLiteralMemberWithComma && contextToken) {
        const changes = textChanges.ChangeTracker.with({ host, formatContext, preferences }, tracker => tracker.insertText(sourceFile, contextToken.end, ","));
        if (changes) {
            return {
                sourceDisplay: undefined,
                codeActions: [{
    ...
    const isJsxOpeningTagName = (contextToken === null || contextToken === void 0 ? void 0 : contextToken.kind) === SyntaxKind.LessThanToken && isJsxOpeningLikeElement(contextToken.parent);
    const { moduleSpecifier, codeAction } = codefix.getImportCompletionAction(targetSymbol, moduleSymbol, data === null || data === void 0 ? void 0 : data.exportMapKey, sourceFile, name, isJsxOpeningTagName, host, program, formatContext, previousToken && isIdentifier(previousToken) ? previousToken.getStart(sourceFile) : position, preferences, cancellationToken);
    Debug.assert(!(data === null || data === void 0 ? void 0 : data.moduleSpecifier) || moduleSpecifier === data.moduleSpecifier);
    return { sourceDisplay: [textPart(moduleSpecifier)], codeActions: [codeAction] };
}
/** @internal */
export function getCompletionEntrySymbol(program, log, sourceFile, position, entryId, host, preferences) {
        ...
        default:
            ...
            return argInfo ?
                ...
                isEqualityOperatorKind(previousToken.kind) && isBinaryExpression(parent) && isEqualityOperatorKind(parent.operatorToken.kind) ?
                    ...
                    checker.getContextualType(previousToken, ContextFlags.Completions) || checker.getContextualType(previousToken);
    }
}
...
function isModuleSymbol(symbol) {
    var _a;
    return !!((_a = symbol.declarations) === null || _a === void 0 ? void 0 : _a.some(d => d.kind === SyntaxKind.SourceFile));
}
function getCompletionData(program, log, sourceFile, compilerOptions, position, preferences, detailsEntryId, host, formatContext, cancellationToken) {
        ...
        if (tag) {
            ...
            if (isJSDocImportTag(tag)) {
                ...
                if (!insideJsDocTagTypeExpression && isJSDocParameterTag(tag) && (nodeIsMissing(tag.name) || tag.name.pos <= position && position <= tag.name.end)) {
                    return { kind: 3 /* CompletionDataKind.JsDocParameterName */, tag };
                }
            }
        }
    ...
    let keywordFilters = 0 /* KeywordCompletionFilters.None */;
    ...
    let completionKind = 5 /* CompletionKind.None */;
    ...
    const seenPropertySymbols = new Set();
    ...
    const literals = !isLiteralExpected ? [] : mapDefined(contextualType && (contextualType.isUnion() ? contextualType.types : [contextualType]), t => t.isLiteral() && !(t.flags & TypeFlags.EnumLiteral) ? t.value : undefined);
    ...
    function getTypeScriptMemberSymbols() {
        ...
        const isTypeLocation = (isImportType && !node.isTypeOf)
            ...
            if (symbol) {
                ...
                if (symbol.flags & (SymbolFlags.Module | SymbolFlags.Enum)) {
                    ...
                    const isValidAccess = isNamespaceName
                        ...
                        ? symbol => { var _a; return !!(symbol.flags & SymbolFlags.Namespace) && !((_a = symbol.declarations) === null || _a === void 0 ? void 0 : _a.every(d => d.parent === node.parent)); }
    ...
    function addTypeProperties(type, insertAwait, insertQuestionDot) {
        ...
        if (isRightOfQuestionDot && some(type.getCallSignatures())) {
            isNewIdentifierLocation = true;
            defaultCommitCharacters !== null && defaultCommitCharacters !== void 0 ? defaultCommitCharacters : (defaultCommitCharacters = allCommitCharacters); // Only invalid commit character here would be `(`.
        }
        const propertyAccess = node.kind === SyntaxKind.ImportType ? node : node.parent;
        if (inCheckedFile) {
        ...
        if (computedPropertyName) {
            ...
            if (firstAccessibleSymbolId && addToSeen(seenPropertySymbols, firstAccessibleSymbolId)) {
                ...
                if (!moduleSymbol ||
                    ...
                    const fileName = isExternalModuleNameRelative(stripQuotes(moduleSymbol.name)) ? (_a = getSourceFileOfModule(moduleSymbol)) === null || _a === void 0 ? void 0 : _a.fileName : undefined;
                    const { moduleSpecifier } = (importSpecifierResolver || (importSpecifierResolver = codefix.createImportSpecifierResolver(sourceFile, program, host, preferences))).getModuleSpecifierForBestExportInfo([{
    ...
    function tryGetGlobalSymbols() {
        const result = tryGetObjectTypeLiteralInTypeArgumentCompletionSymbols()
        ...
        for (let i = 0; i < symbols.length; i++) {
            ...
            if (typeOnlyAliasNeedsPromotion && !(symbol.flags & SymbolFlags.Value)) {
                ...
                if (typeOnlyAliasDeclaration) {
                    const origin = { kind: 64 /* SymbolOriginInfoKind.TypeOnlyAlias */, declaration: typeOnlyAliasDeclaration };
    ...
    function shouldOfferImportCompletions() {
        ...
        return ((_a = program.getSymlinkCache) === null || _a === void 0 ? void 0 : _a.call(program).hasAnySymlinks()) ||
    ...
    function collectAutoImports() {
        ...
        if (!shouldOfferImportCompletions())
            return;
        Debug.assert(!(detailsEntryId === null || detailsEntryId === void 0 ? void 0 : detailsEntryId.data), "Should not run 'collectAutoImports' when faster path is available via `data`");
        ...
        const packageJsonAutoImportProvider = (_b = host.getPackageJsonAutoImportProvider) === null || _b === void 0 ? void 0 : _b.call(host);
        const packageJsonFilter = detailsEntryId ? undefined : createPackageJsonImportFilter(sourceFile, preferences, host);
        resolvingModuleSpecifiers("collectAutoImports", host, importSpecifierResolver || (importSpecifierResolver = codefix.createImportSpecifierResolver(sourceFile, program, host, preferences)), program, position, preferences, !!importStatementCompletion, isValidTypeOnlyAliasUseSite(location), context => {
            exportInfo.search(sourceFile.path, 
            /*preferCapitalized*/ isRightOfOpenTag, (symbolName, targetFlags) => {
        ...
        function isImportableExportInfo(info) {
            return isImportable(info.isFromPackageJson ? packageJsonAutoImportProvider : program, sourceFile, tryCast(info.moduleSymbol.valueDeclaration, isSourceFile), info.moduleSymbol, preferences, packageJsonFilter, getModuleSpecifierResolutionHost(info.isFromPackageJson), moduleSpecifierCache);
        }
    }
    ...
    function collectObjectLiteralMethodSymbols(members, enclosingDeclaration) {
        ...
        members.forEach(member => {
            ...
            const origin = Object.assign({ kind: 128 /* SymbolOriginInfoKind.ObjectLiteralMethod */ }, entryProps);
    ...
    function isInStringOrRegularExpressionOrTemplateLiteral(contextToken) {
        ...
        return (isRegularExpressionLiteral(contextToken) || isStringTextContainingNode(contextToken)) && (rangeContainsPositionExclusive(contextToken, position) ||
    ...
    function tryGetObjectLikeCompletionSymbols() {
        if ((contextToken === null || contextToken === void 0 ? void 0 : contextToken.kind) === SyntaxKind.DotDotDotToken)
            return 0 /* GlobalsSearch.Continue */;
    ...
    function tryGetImportOrExportClauseCompletionSymbols() {
        ...
        if (!moduleSpecifier) {
            isNewIdentifierLocation = true;
            return namedImportsOrExports.kind === SyntaxKind.NamedImports ? 2 /* GlobalsSearch.Fail */ : 0 /* GlobalsSearch.Continue */;
        }
    ...
    function tryGetLocalNamedExportCompletionSymbols() {
        ...
        completionKind = 5 /* CompletionKind.None */;
        isNewIdentifierLocation = false;
        (_a = localsContainer.locals) === null || _a === void 0 ? void 0 : _a.forEach((symbol, name) => {
            var _a, _b;
            symbols.push(symbol);
    ...
    function tryGetClassLikeCompletionSymbols() {
        ...
        // Declaring new property/method/accessor
        isNewIdentifierLocation = true;
        keywordFilters = contextToken.kind === SyntaxKind.AsteriskToken ? 0 /* KeywordCompletionFilters.None */ :
            isClassLike(decl) ? 2 /* KeywordCompletionFilters.ClassElementKeywords */ : 3 /* KeywordCompletionFilters.InterfaceElementKeywords */;
        ...
        if (!(classElementModifierFlags & ModifierFlags.Private)) {
            ...
            forEach(symbols, (symbol, index) => {
                ...
                if (declaration && isClassElement(declaration) && declaration.name && isComputedPropertyName(declaration.name)) {
                    const origin = {
                        kind: 512 /* SymbolOriginInfoKind.ComputedPropertyName */,
    ...
    function tryGetFunctionLikeBodyCompletionContainer(contextToken) {
        if (contextToken) {
            let prev;
            const container = findAncestor(contextToken.parent, (node) => {
                if (isClassLike(node)) {
                    return "quit";
                }
                ...
                case SyntaxKind.JsxSpreadAttribute:
                    if (parent && (parent.kind === SyntaxKind.JsxSelfClosingElement || parent.kind === SyntaxKind.JsxOpeningElement)) {
                        if (contextToken.kind === SyntaxKind.GreaterThanToken) {
                            ...
                            if (!parent.typeArguments || (precedingToken && precedingToken.kind === SyntaxKind.SlashToken))
                                break;
                        }
                        return parent;
                    }
    ...
    function isInDifferentLineThanContextToken(contextToken, position) {
    ...
    function isSolelyIdentifierDefinitionLocation(contextToken) {
        ...
        if (ancestorPropertyDeclaraion
            ...
            if (isPreviousPropertyDeclarationTerminated(contextToken, previousToken.end)) {
                ...
                && (isInitializedProperty(ancestorPropertyDeclaraion)
    ...
    function setSortTextToOptionalMember() {
        symbols.forEach(m => {
            ...
            if (m.flags & SymbolFlags.Optional) {
                ...
                symbolToSortTextMap[symbolId] = (_a = symbolToSortTextMap[symbolId]) !== null && _a !== void 0 ? _a : SortText.OptionalMember;
            }
        });
    }
    ...
    function transformObjectLiteralMembersSortText(start) {
        ...
        for (let i = start; i < symbols.length; i++) {
            ...
            const displayName = getCompletionEntryDisplayNameForSymbol(symbol, target, origin, 0 /* CompletionKind.ObjectPropertyDeclaration */, 
            /*jsxIdentifierExpected*/ false);
            if (displayName) {
                const originalSortText = (_a = symbolToSortTextMap[symbolId]) !== null && _a !== void 0 ? _a : SortText.LocationPriority;
                const { name } = displayName;
        ...
        switch (contextToken.kind) {
            ...
            case SyntaxKind.Identifier:
                if (contextToken.text === "async" && isShorthandPropertyAssignment(contextToken.parent)) {
                    ...
                    if ((ancestorNode === null || ancestorNode === void 0 ? void 0 : ancestorNode.getLastToken(sourceFile)) === contextToken && isObjectLiteralExpression(ancestorNode.parent)) {
                        return ancestorNode.parent;
                    }
                }
                break;
            default:
                if (((_a = parent.parent) === null || _a === void 0 ? void 0 : _a.parent) && (isMethodDeclaration(parent.parent) || isGetAccessorDeclaration(parent.parent) || isSetAccessorDeclaration(parent.parent)) && isObjectLiteralExpression(parent.parent.parent)) {
                    return parent.parent.parent;
                }
                ...
                const ancestorNode = findAncestor(parent, isPropertyAssignment);
                if (contextToken.kind !== SyntaxKind.ColonToken && (ancestorNode === null || ancestorNode === void 0 ? void 0 : ancestorNode.getLastToken(sourceFile)) === contextToken &&
...
const allKeywordsCompletions = memoize(() => {
    const res = [];
...
function keywordForNode(node) {
    var _a;
    return isIdentifier(node) ? (_a = identifierToKeywordKind(node)) !== null && _a !== void 0 ? _a : SyntaxKind.Unknown : node.kind;
}
...
function getJsDocTagAtPosition(node, position) {
    return findAncestor(node, n => isJSDocTag(n) && rangeContainsPosition(n, position) ? true :
        isJSDoc(n) ? "quit" : false);
}
/** @internal */
export function getPropertiesForObjectExpression(contextualType, completionsType, obj, checker) {
    const hasCompletionsType = completionsType && completionsType !== contextualType;
    const promiseFilteredContextualType = checker.getUnionType(filter(contextualType.flags & TypeFlags.Union ?
...
function getApparentProperties(type, node, checker) {
    ...
    return checker.getAllPossiblePropertiesOfTypes(filter(type.types, memberType => !(memberType.flags & TypeFlags.Primitive
    ...
    switch (contextToken.kind) {
        ...
        default:
            if (isObjectTypeDeclaration(location)) {
                ...
                return (isValidKeyword(contextToken.kind) || contextToken.kind === SyntaxKind.AsteriskToken || isIdentifier(contextToken) && isValidKeyword((_a = identifierToKeywordKind(contextToken)) !== null && _a !== void 0 ? _a : SyntaxKind.Unknown))
...
function getImportStatementCompletionInfo(contextToken, sourceFile) {
    ...
    return {
        ...
        isTopLevelTypeOnly: !!((_b = (_a = tryCast(candidate, isImportDeclaration)) === null || _a === void 0 ? void 0 : _a.importClause) === null || _b === void 0 ? void 0 : _b.isTypeOnly) || !!((_c = tryCast(candidate, isImportEqualsDeclaration)) === null || _c === void 0 ? void 0 : _c.isTypeOnly),
    ...
    function getCandidate() {
        ...
        if (isNamedImports(parent) || isNamespaceImport(parent)) {
            if (!parent.parent.isTypeOnly && (contextToken.kind === SyntaxKind.OpenBraceToken ||
...
function getSingleLineReplacementSpanForImportCompletionNode(node) {
    ...
    if (!node)
        return undefined;
    const top = (_a = findAncestor(node, or(isImportDeclaration, isImportEqualsDeclaration, isJSDocImportTag))) !== null && _a !== void 0 ? _a : node;
...
function getPotentiallyInvalidImportSpecifier(namedBindings) {
    var _a;
    return find((_a = tryCast(namedBindings, isNamedImports)) === null || _a === void 0 ? void 0 : _a.elements, e => {
        var _a;
        return !e.propertyName &&
            isStringANonContextualKeyword(e.name.text) &&
            ((_a = findPrecedingToken(e.name.pos, namedBindings.getSourceFile(), namedBindings)) === null || _a === void 0 ? void 0 : _a.kind) !== SyntaxKind.CommaToken;
    });
}
...
function isModuleSpecifierMissingOrEmpty(specifier) {
    ...
    if (nodeIsMissing(specifier))
        return true;
    return !((_a = tryCast(isExternalModuleReference(specifier) ? specifier.expression : specifier, isStringLiteralLike)) === null || _a === void 0 ? void 0 : _a.text);
}
function getClosestSymbolDeclaration(contextToken, location) {
    if (!contextToken)
        return;
    let closestDeclaration = findAncestor(contextToken, node => isFunctionBlock(node) || isArrowFunctionBody(node) || isBindingPattern(node)
        ? "quit"
        : ((isParameter(node) || isTypeParameterDeclaration(node)) && !isIndexSignatureDeclaration(node.parent)));
    if (!closestDeclaration) {
        closestDeclaration = findAncestor(location, node => isFunctionBlock(node) || isArrowFunctionBody(node) || isBindingPattern(node)
            ? "quit"
...
function symbolCanBeReferencedAtTypeLocation(symbol, checker, seenModules = new Set()) {

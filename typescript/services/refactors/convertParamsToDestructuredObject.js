...
function doChange(sourceFile, program, host, changes, functionDeclaration, groupedReferences) {
    ...
    for (const call of functionCalls) {
        if (call.arguments && call.arguments.length) {
            ...
            changes.replaceNodeRange(getSourceFileOfNode(call), first(call.arguments), last(call.arguments), newArgument, { leadingTriviaOption: textChanges.LeadingTriviaOption.IncludeAll, trailingTriviaOption: textChanges.TrailingTriviaOption.Include });
        }
    }
...
function getGroupedReferences(functionDeclaration, program, cancellationToken) {
    ...
    const references = flatMap(names, /*mapfn*/ /*mapfn*/ name => FindAllReferences.getReferenceEntriesForNode(-1, name, program, program.getSourceFiles(), cancellationToken));
    const groupedReferences = groupReferences(references);
    if (!every(groupedReferences.declarations, /*callback*/ /*callback*/ decl => contains(names, decl))) {
        groupedReferences.valid = false;
    }
    return groupedReferences;
    function groupReferences(referenceEntries) {
        const classReferences = { accessExpressions: [], typeUsages: [] };
        const groupedReferences = { functionCalls: [], declarations: [], classReferences, valid: true };
...
function getSymbolForContextualType(node, checker) {
    ...
    if (element) {
        ...
        const symbol = contextualType === null || contextualType === void 0 ? void 0 : contextualType.getSymbol();
...
function isValidParameterNodeArray(parameters, checker) {
    return getRefactorableParametersLength(parameters) >= minimumParameterLength
        && every(parameters, /*callback*/ /*callback*/ paramDecl => isValidParameterDeclaration(paramDecl, checker));
}

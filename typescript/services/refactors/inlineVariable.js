...
registerRefactor(refactorName, {
    ...
    getAvailableActions(context) {
        const { file, program, preferences, startPosition, triggerReason, } = context;
        ...
        if (preferences.provideRefactorNotApplicableReason) {
            return [{
                    name: refactorName,
                    description: refactorDescription,
                    actions: [Object.assign(Object.assign({}, inlineVariableAction), { notApplicableReason: info.error })],
                }];
        }
        return emptyArray;
    },
...
function getInliningInfo(file, startPosition, tryWithReferenceToken, program) {
    ...
    if (isInitializedVariable(parent) && isVariableDeclarationInVariableStatement(parent) && isIdentifier(parent.name)) {
        ...
        if (((_a = checker.getMergedSymbol(parent.symbol).declarations) === null || _a === void 0 ? void 0 : _a.length) !== 1) {
    ...
    if (tryWithReferenceToken) {
        ...
        // Don't inline the variable if it has multiple declarations.
        if (((_b = definition === null || definition === void 0 ? void 0 : definition.declarations) === null || _b === void 0 ? void 0 : _b.length) !== 1) {
            return { error: getLocaleSpecificMessage(Diagnostics.Variables_with_multiple_declarations_cannot_be_inlined) };
        }
...
function getReferenceNodes(declaration, checker, file) {
    const references = [];
    const cannotInline = FindAllReferences.Core.eachSymbolReferenceInFile(declaration.name, checker, file, ref => {
...
function replaceTemplateStringVariableWithLiteral(tracker, sourceFile, reference, replacement) {
    ...
    tracker.replaceRangeWithText(sourceFile, {

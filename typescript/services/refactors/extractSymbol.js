 ...
 * @internal
 */
export function getRefactorActionsToExtractSymbol(context) {
    ...
    if (targetRange === undefined) {
        ...
        if (refactorKindBeginsWith(extractFunctionAction.kind, requestedRefactor)) {
            errors.push({
                ...
                actions: [Object.assign(Object.assign({}, extractFunctionAction), { notApplicableReason: getStringError(rangeToExtract.errors) })],
            });
        }
        if (refactorKindBeginsWith(extractConstantAction.kind, requestedRefactor)) {
            errors.push({
                ...
                actions: [Object.assign(Object.assign({}, extractConstantAction), { notApplicableReason: getStringError(rangeToExtract.errors) })],
            });
        }
        return errors;
    }
 ...
 * @internal
 */
export function getRefactorEditsToExtractSymbol(context, actionName) {
 ...
 * @internal
 */
export function getRangeToExtract(sourceFile, span, invoked = true) {
    const { length } = span;
    if (length === 0 && !invoked) {
    ...
    function checkNode(nodeToCheck) {
        ...
        let errors;
        let permittedJumps = 4 /* PermittedJumps.Return */;
        let seenLabels;
        visit(nodeToCheck);
            ...
            switch (node.kind) {
                ...
                case SyntaxKind.ContinueStatement: {
                    ...
                    if (label) {
                        ...
                        if (!(permittedJumps & (node.kind === SyntaxKind.BreakStatement ? 1 /* PermittedJumps.Break */ : 2 /* PermittedJumps.Continue */))) {
...
function extractFunctionInScope(node, scope, { usages: usagesInScope, typeParameterUsages, substitutions }, exposedVariableDeclarations, range, context) {
    ...
    const typeParameters = typeParametersAndDeclarations.length === 0
        ? undefined
        : mapDefined(typeParametersAndDeclarations, ({ declaration }) => declaration);
    ...
    if (isClassLike(scope)) {
        ...
        const modifiers = isJS ? [] : [factory.createModifier(SyntaxKind.PrivateKeyword)];
        ...
        newFunction = factory.createMethodDeclaration(modifiers.length ? modifiers : undefined, range.facts & RangeFacts.IsGenerator ? factory.createToken(SyntaxKind.AsteriskToken) : undefined, functionName, 
        ...
        if (callThis) {
            ...
            /*dotDotDotToken*/ undefined, 
            /*name*/ "this", 
            /*questionToken*/ undefined, checker.typeToTypeNode(checker.getTypeAtLocation(range.thisNode), scope, NodeBuilderFlags.NoTruncation, InternalNodeBuilderFlags.AllowUnresolvedNames), 
            /*initializer*/ undefined));
        }
        newFunction = factory.createFunctionDeclaration(range.facts & RangeFacts.IsAsyncFunction ? [factory.createToken(SyntaxKind.AsyncKeyword)] : undefined, range.facts & RangeFacts.IsGenerator ? factory.createToken(SyntaxKind.AsteriskToken) : undefined, functionName, typeParameters, parameters, returnType, body);
    }
    ...
    if (exposedVariableDeclarations.length && !writes) {
        ...
        if (exposedVariableDeclarations.length === 1) {
            ...
            /*modifiers*/ undefined, factory.createVariableDeclarationList([factory.createVariableDeclaration(getSynthesizedDeepClone(variableDeclaration.name), /*exclamationToken*/ undefined, /*type*/ getSynthesizedDeepClone(variableDeclaration.type), /*initializer*/ call)], variableDeclaration.parent.flags)));
        }
        else {
            ...
            for (const variableDeclaration of exposedVariableDeclarations) {
                ...
                const variableType = checker.typeToTypeNode(checker.getBaseTypeOfLiteralType(checker.getTypeAtLocation(variableDeclaration)), scope, NodeBuilderFlags.NoTruncation, InternalNodeBuilderFlags.AllowUnresolvedNames);
            ...
            const typeLiteral = sawExplicitType ? factory.createTypeLiteralNode(typeElements) : undefined;
            if (typeLiteral) {
...
function extractConstantInScope(node, scope, { substitutions }, rangeFacts, context) {
    ...
    if (isClassLike(scope)) {
        ...
        /*questionOrExclamationToken*/ undefined, variableType, initializer);
        let localReference = factory.createPropertyAccessExpression(rangeFacts & RangeFacts.InStaticRegion
        ...
        if (isArrowFunction(initializer)) {
            ...
            if (functionSignature && !!functionSignature.thisParameter) {
                ...
                if ((!firstParameter || (isIdentifier(firstParameter.name) && firstParameter.name.escapedText !== "this"))) {
                    ...
                    /*questionToken*/ undefined, checker.typeToTypeNode(thisType, scope, NodeBuilderFlags.NoTruncation, InternalNodeBuilderFlags.AllowUnresolvedNames)));
                }
            }
...
function collectReadsAndWrites(targetRange, scopes, enclosingTextRange, sourceFile, checker, cancellationToken) {
    const allTypeParameterUsages = new Map(); // Key is type ID
    ...
    const exposedVariableSymbolSet = new Map(); // Key is symbol ID
    ...
    if (allTypeParameterUsages.size > 0) {
        const seenTypeParameterUsages = new Map(); // Key is type ID
        let i = 0;
    ...
    function collectUsages(node, valueUsage = 1 /* Usage.Read */) {
        if (inGenericContext) {
    ...
    function tryReplaceWithQualifiedNameOrPropertyAccess(symbol, scopeDecl, isTypeNode) {
        ...
        return isTypeNode
            ? factory.createQualifiedName(prefix, factory.createIdentifier(symbol.name))
            : factory.createPropertyAccessExpression(prefix, symbol.name);
    }
}

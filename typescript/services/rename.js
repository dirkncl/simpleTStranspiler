...
function getRenameInfoForNode(node, typeChecker, sourceFile, program, preferences) {
    ...
    if (!symbol) {
        if (isStringLiteralLike(node)) {
            ...
            if (type && ((type.flags & TypeFlags.StringLiteral) || ((type.flags & TypeFlags.Union) && every(type.types, type => !!(type.flags & TypeFlags.StringLiteral))))) {
...
/** @internal */
export function nodeIsEligibleForRename(node) {
    switch (node.kind) {

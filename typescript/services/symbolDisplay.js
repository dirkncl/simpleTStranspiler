...
/** @internal */
export function getSymbolKind(typeChecker, symbol, location) {
...
function getSymbolKindOfConstructorPropertyMethodAccessorFunctionOrVar(typeChecker, symbol, location) {
    ...
    if (flags & SymbolFlags.Property) {
        if (flags & SymbolFlags.Transient && symbol.links.checkFlags & CheckFlags.Synthetic) {
...
/** @internal */
export function getSymbolModifiers(typeChecker, symbol) {
    if (!symbol) {
...
function getSymbolDisplayPartsDocumentationAndSymbolKindWorker(typeChecker, symbol, sourceFile, enclosingDeclaration, location, type, semanticMeaning, alias) {
    ...
    if (symbolKind !== ScriptElementKind.unknown || symbolFlags & SymbolFlags.Class || symbolFlags & SymbolFlags.Alias) {
        ...
        if (symbolKind === ScriptElementKind.memberGetAccessorElement || symbolKind === ScriptElementKind.memberSetAccessorElement) {
            const declaration = find(symbol.declarations, (declaration) => declaration.name === location
        ...
        let signature;
        type !== null && type !== void 0 ? type : (type = isThisExpression ? typeChecker.getTypeAtLocation(location) : typeChecker.getTypeOfSymbolAtLocation(symbol, location));
    ...
    if (symbolFlags & SymbolFlags.EnumMember) {
        ...
        if ((declaration === null || declaration === void 0 ? void 0 : declaration.kind) === SyntaxKind.EnumMember) {
        ...
        if (!hasAddedSymbolInfo || documentation.length === 0 && tags.length === 0) {
            ...
            if (resolvedSymbol !== symbol && resolvedSymbol.declarations && resolvedSymbol.declarations.length > 0) {
                ...
                if (declarationName && !hasAddedSymbolInfo) {
                    ...
                    const resolvedInfo = getSymbolDisplayPartsDocumentationAndSymbolKindWorker(typeChecker, resolvedSymbol, getSourceFileOfNode(resolvedNode), enclosingDeclaration, declarationName, type, semanticMeaning, shouldUseAliasName ? symbol : resolvedSymbol);
    ...
    function addSignatureDisplayParts(signature, allSignatures, flags = TypeFormatFlags.None) {
...
/** @internal */
export function getSymbolDisplayPartsDocumentationAndSymbolKind(typeChecker, symbol, sourceFile, enclosingDeclaration, location, semanticMeaning = getMeaningFromLocation(location), alias) {

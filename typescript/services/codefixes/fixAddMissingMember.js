        ...
        return createCombinedCodeActions(textChanges.ChangeTracker.with(context, changes => {
            ...
            typeDeclToMembers.forEach((infos, declaration) => {
                ...
                for (const info of infos) {
                    ...
                    if (supers === null || supers === void 0 ? void 0 : supers.some(superClassOrInterface => {
...
function getInfo(sourceFile, tokenPos, errorCode, checker, program) {
    ...
    if (isIdentifier(token) && hasInitializer(parent) && parent.initializer && isObjectLiteralExpression(parent.initializer)) {
        const targetType = (_a = (checker.getContextualType(token) || checker.getTypeAtLocation(token))) === null || _a === void 0 ? void 0 : _a.getNonNullableType();
    ...
    if (isIdentifier(token)) {
        const type = (_b = checker.getContextualType(token)) === null || _b === void 0 ? void 0 : _b.getNonNullableType();
    ...
    if (isIdentifier(token) && isCallExpression(parent.parent)) {
        ...
        const moduleDeclarationSourceFile = moduleDeclaration === null || moduleDeclaration === void 0 ? void 0 : moduleDeclaration.getSourceFile();
...
function createActionsForAddMissingMemberInTypeScriptFile(context, { parentDeclaration, declSourceFile, modifierFlags, token }) {
    ...
    const addPropertyDeclarationChanges = (modifierFlags) => textChanges.ChangeTracker.with(context, t => addPropertyDeclaration(t, declSourceFile, parentDeclaration, memberName, typeNode, modifierFlags));
...
function getActionsForMissingMethodDeclaration(context, info) {
    ...
    const addMethodDeclarationChanges = (modifierFlags) => textChanges.ChangeTracker.with(context, t => addMethodDeclaration(context, t, call, token, modifierFlags, parentDeclaration, declSourceFile));
...
function tryGetValueFromType(context, checker, importAdder, quotePreference, type, enclosingDeclaration) {
    ...
    if (type.flags & TypeFlags.StringLiteral) {
        return factory.createStringLiteral(type.value, /* isSingleQuote */ quotePreference === QuotePreference.Single);
    }
    ...
    if (getObjectFlags(type) & ObjectFlags.Class) {
        ...
        if (classDeclaration === undefined || hasAbstractModifier(classDeclaration))
            return createUndefined();

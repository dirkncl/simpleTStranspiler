...
registerCodeFix({
    ...
    getCodeActions(context) {
        ...
        if (changes.length > 0) {
            return [
                createCodeFixAction(fixId, changes, Diagnostics.Convert_typedef_to_TypeScript_type, fixId, Diagnostics.Convert_all_typedef_to_TypeScript_types),
            ];
        }
    },
...
function doChange(changes, node, sourceFile, newLine, fixAll = false) {
...
function createDeclaration(tag) {
    ...
    if (!typeExpression)
        return;
    const typeName = (_a = tag.name) === null || _a === void 0 ? void 0 : _a.getText();
    if (!typeName)
        return;
...
function createInterfaceForTypeLiteral(typeName, typeLiteral) {
    ...
    /*modifiers*/ undefined, typeName, 
    /*typeParameters*/ undefined, 
    /*heritageClauses*/ undefined, propertySignatures);
}
function createTypeAliasForTypeExpression(typeName, typeExpression) {
    ...
    /*modifiers*/ undefined, factory.createIdentifier(typeName), 
    /*typeParameters*/ undefined, typeReference);
}
function createSignatureFromTypeLiteral(typeLiteral) {
    ...
    const getSignature = (tag) => {
        ...
        const type = (_a = tag.typeExpression) === null || _a === void 0 ? void 0 : _a.type;
        ...
        if (typeReference && name) {
            ...
            /*modifiers*/ undefined, name, questionToken, typeReference);
        }
    };
...
/** @internal */
export function getJSDocTypedefNodes(node) {
    if (hasJSDocNodes(node)) {
        return flatMap(node.jsDoc, doc => { var _a; return (_a = doc.tags) === null || _a === void 0 ? void 0 : _a.filter(tag => isJSDocTypedefTag(tag)); });
    }
    return [];
}

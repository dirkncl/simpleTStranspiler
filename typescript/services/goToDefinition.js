...
export function getDefinitionAtPosition(program, sourceFile, position, searchOtherFilesOnly, stopAtAlias) {
    ...
    if (searchOtherFilesOnly && failedAliasResolution) {
        ...
        const importDeclaration = forEach([node, ...(symbol === null || symbol === void 0 ? void 0 : symbol.declarations) || emptyArray], n => findAncestor(n, isAnyImportOrBareOrAccessedRequire));
    ...
    if (!symbol && isModuleSpecifierLike(fallbackNode)) {
        ...
        const ref = (_a = program.getResolvedModuleFromModuleSpecifier(fallbackNode, sourceFile)) === null || _a === void 0 ? void 0 : _a.resolvedModule;
        if (ref) {
            return [{
    ...
    if (node.parent.kind === SyntaxKind.ShorthandPropertyAssignment) {
        ...
        const definitions = (shorthandSymbol === null || shorthandSymbol === void 0 ? void 0 : shorthandSymbol.declarations) ? shorthandSymbol.declarations.map(decl => createDefinitionInfo(decl, typeChecker, shorthandSymbol, node, /*unverified*/ false, failedAliasResolution)) : emptyArray;
...
function symbolMatchesSignature(s, calledDeclaration) {
    ...
    return s === calledDeclaration.symbol
        ...
        || (!isCallLikeExpression(calledDeclaration.parent) && s === ((_a = tryCast(calledDeclaration.parent, canHaveSymbol)) === null || _a === void 0 ? void 0 : _a.symbol));
}
...
/** @internal */
export function getReferenceAtPosition(sourceFile, position, program) {
    var _a, _b;
    ...
    if (typeReferenceDirective) {
        const reference = (_a = program.getResolvedTypeReferenceDirectiveFromTypeReferenceDirective(typeReferenceDirective, sourceFile)) === null || _a === void 0 ? void 0 : _a.resolvedTypeReferenceDirective;
    ...
    if (sourceFile.imports.length || sourceFile.moduleAugmentations.length) {
        ...
        if (isModuleSpecifierLike(node) && isExternalModuleNameRelative(node.text) && (resolution = program.getResolvedModuleFromModuleSpecifier(node, sourceFile))) {
            const verifiedFileName = (_b = resolution.resolvedModule) === null || _b === void 0 ? void 0 : _b.resolvedFileName;
...
function getFirstTypeArgumentDefinitions(typeChecker, type, node, failedAliasResolution) {
    var _a, _b;
    if (!!(getObjectFlags(type) & ObjectFlags.Reference) && shouldUnwrapFirstTypeArgumentTypeDefinitionFromTypeReference(typeChecker, type)) {
    ...
    if ((getObjectFlags(type) & ObjectFlags.Mapped) &&
        type.target &&
        shouldUnwrapFirstTypeArgumentTypeDefinitionFromAlias(typeChecker, type.target)) {
        const declaration = (_b = (_a = type.aliasSymbol) === null || _a === void 0 ? void 0 : _a.declarations) === null || _b === void 0 ? void 0 : _b[0];
...
/// Goto type
/** @internal */
export function getTypeDefinitionAtPosition(typeChecker, sourceFile, position) {
    const node = getTouchingPropertyName(sourceFile, position);
    if (node === sourceFile) {
        return undefined;
    }
...
/** @internal */
export function getDefinitionAndBoundSpan(program, sourceFile, position) {
...
function getSymbol(node, checker, stopAtAlias) {
    ...
    if ((symbol === null || symbol === void 0 ? void 0 : symbol.declarations) && symbol.flags & SymbolFlags.Alias && !stopAtAlias && shouldSkipAlias(node, symbol.declarations[0])) {
 ...
 * @internal
 */
export function createDefinitionInfo(declaration, checker, symbol, node, unverified, failedAliasResolution) {
...
function createDefinitionInfoFromName(checker, declaration, symbolKind, symbolName, containerName, unverified, failedAliasResolution, textSpan) {
    ...
    return Object.assign(Object.assign({ fileName: sourceFile.fileName, textSpan, kind: symbolKind, name: symbolName, containerKind: undefined, // TODO: GH#18217
        containerName }, FindAllReferences.toContextSpan(textSpan, sourceFile, FindAllReferences.getContextNode(declaration))), { isLocal: !isDefinitionVisible(checker, declaration), isAmbient: !!(declaration.flags & NodeFlags.Ambient), unverified,
        failedAliasResolution });
}
function createDefinitionInfoFromSwitch(statement, sourceFile) {
    ...
    return Object.assign(Object.assign({ fileName: sourceFile.fileName, textSpan, kind: ScriptElementKind.keyword, name: "switch", containerKind: undefined, containerName: "" }, FindAllReferences.toContextSpan(textSpan, sourceFile, keyword)), { isLocal: true, isAmbient: false, unverified: false, failedAliasResolution: undefined });
}
...
function getAncestorCallLikeExpression(node) {
    ...
    const callLike = target === null || target === void 0 ? void 0 : target.parent;

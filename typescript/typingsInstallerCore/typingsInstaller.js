...
/** @internal */
export function installNpmPackages(npmPath, tsVersion, packageNames, install) {
    let hasError = false;
...
/** @internal */
export function getNpmCommandForInstallation(npmPath, tsVersion, packageNames, remaining) {
...
export class TypingsInstaller {
    ...
    install(req) {
        ...
        const discoverTypingsResult = JsTyping.discoverTypings(this.installTypingHost, this.log.isEnabled() ? (s => this.log.writeLine(s)) : undefined, req.fileNames, req.projectRootPath, this.safeList, this.packageNameToTypingLocation, req.typeAcquisition, req.unresolvedImports, this.typesRegistry, req.compilerOptions);
    ...
    processCacheLocation(cacheLocation) {
        ...
        if (this.installTypingHost.fileExists(packageJson) && this.installTypingHost.fileExists(packageLockJson)) {
            const npmConfig = JSON.parse(this.installTypingHost.readFile(packageJson)); // TODO: GH#18217
            const npmLock = JSON.parse(this.installTypingHost.readFile(packageLockJson)); // TODO: GH#18217
            if (this.log.isEnabled()) {
            ...
            if (npmConfig.devDependencies && npmLock.dependencies) {
                for (const key in npmConfig.devDependencies) {
                    ...
                    const newTyping = { typingLocation: typingFile, version: new Version(version) };
...
/** @internal */
export function typingsName(packageName) {

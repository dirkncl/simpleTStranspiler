...
function generateOptionOutput(sys, option, rightAlignOfLeft, leftAlignOfRight) {
    ...
    const defaultValueDescription = typeof option.defaultValueDescription === "object"
        ...
        : formatDefaultValue(option.defaultValueDescription, option.type === "list" || option.type === "listOrElement" ? option.element.type : option.type);
    const terminalWidth = (_b = (_a = sys.getWidthOfTerminal) === null || _a === void 0 ? void 0 : _a.call(sys)) !== null && _b !== void 0 ? _b : 0;
    ...
    function showAdditionalInfoOutput(valueCandidates, option) {
        ...
        if (option.category === Diagnostics.Command_line_Options)
            return false;
        if (contains(ignoreValues, valueCandidates === null || valueCandidates === void 0 ? void 0 : valueCandidates.possibleValues) && contains(ignoredDescriptions, defaultValueDescription)) {
            return false;
        }
        return true;
    }
            ...
            switch (option.type) {
                ...
                default:
                    ...
                    option.type.forEach((value, name) => {
                        var _a;
                        if (!((_a = option.deprecatedKeys) === null || _a === void 0 ? void 0 : _a.has(name))) {
...
function generateSectionOptionsOutput(sys, sectionName, options, subCategory, beforeOptionsDescription, afterOptionsDescription) {
    ...
    for (const option of options) {
        ...
        const optionsOfCurCategory = (_a = categoryMap.get(curCategory)) !== null && _a !== void 0 ? _a : [];
...
function printEasyHelp(sys, simpleOptions) {
    const colors = createColors(sys);
    let output = [...getHeader(sys, `${getDiagnosticText(Diagnostics.tsc_Colon_The_TypeScript_Compiler)} - ${getDiagnosticText(Diagnostics.Version_0, version)}`)];
...
function executeCommandLineWorker(sys, cb, commandLine) {
    ...
    if (configFileName) {
        ...
        if (isWatchSet(commandLineOptions)) {
            ...
            performIncrementalCompilation(sys, cb, reportDiagnostic, Object.assign(Object.assign({}, commandLine), { options: commandLineOptions }));
        }
        else {
            performCompilation(sys, cb, reportDiagnostic, Object.assign(Object.assign({}, commandLine), { options: commandLineOptions }));
        }
    }
}
/** Returns true if commandline is --build and needs to be parsed useing parseBuildCommand */
export function isBuildCommand(commandLineArgs) {
...
/** @internal */
export function executeCommandLine(system, cb, commandLineArgs) {
...
function performBuild(sys, cb, buildOptions, watchOptions, projects, errors) {
    ...
    if (buildOptions.watch) {
        ...
        buildHost.onWatchStatusChange = (d, newLine, options, errorCount) => {
            onWatchStatusChange === null || onWatchStatusChange === void 0 ? void 0 : onWatchStatusChange(d, newLine, options, errorCount);
            if (reportBuildStatistics && (d.code === Diagnostics.Found_0_errors_Watching_for_file_changes.code ||
...
function createSolutionPerfomrance() {
    ...
    function addAggregateStatistic(s) {
        const existing = statistics === null || statistics === void 0 ? void 0 : statistics.get(s.name);
        if (existing) {
            ...
            (statistics !== null && statistics !== void 0 ? statistics : (statistics = new Map())).set(s.name, s);
        }
    }
...
function reportSolutionBuilderTimes(builder, solutionPerformance) {
    ...
    const statistics = [];
    statistics.push({ name: "Projects in scope", value: getBuildOrderFromAnyBuildOrder(builder.getBuildOrder()).length, type: StatisticType.count });
...
function writeConfigFile(sys, reportDiagnostic, options, fileNames) {
    ...
    if (sys.fileExists(file)) {
        ...
        const output = [sys.newLine, ...getHeader(sys, "Created a new tsconfig.json with:")];

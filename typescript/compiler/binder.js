...
/** @internal */
export var ModuleInstanceState;
...
/** @internal */
export function getModuleInstanceState(node, visited) {
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.ExportDeclaration:
            ...
            if (!exportDeclaration.moduleSpecifier && exportDeclaration.exportClause && exportDeclaration.exportClause.kind === SyntaxKind.NamedExports) {
                let state = 0 /* ModuleInstanceState.NonInstantiated */;
            ...
            forEachChild(node, n => {
                ...
                switch (childState) {
                    ...
                    case 1 /* ModuleInstanceState.Instantiated */:
                        ...
                        state = 1 /* ModuleInstanceState.Instantiated */;
                        return true;
                    default:
...
function getModuleInstanceStateForAliasTarget(specifier, visited) {
    ...
    if (name.kind !== SyntaxKind.Identifier) {
        return 1 /* ModuleInstanceState.Instantiated */; // Skip for invalid syntax like this: export { "x" }
    }
            ...
            for (const statement of statements) {
                if (nodeHasName(statement, name)) {
                    ...
                    if (statement.kind === SyntaxKind.ImportEqualsDeclaration) {
                        ...
                        //   import x = mod.x;
                        //   export { x };
                        found = 1 /* ModuleInstanceState.Instantiated */;
                    }
                }
            }
            if (found !== undefined) {
...
/** @internal */
export function createFlowNode(flags, node, antecedent) {
...
function createBinder() {
    ...
    function bindSourceFile(f, opts) {
        ...
        if (!file.locals) {
            tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Bind, "bindSourceFile", { path: file.path }, /*separateBeginAndEnd*/ true);
            bind(file);
    ...
    function bindContainer(node, containerFlags) {
        ...
        if (containerFlags & 4 /* ContainerFlags.IsControlFlowContainer */) {
            ...
            const isImmediatelyInvoked = (containerFlags & 16 /* ContainerFlags.IsFunctionExpression */ &&
                !hasSyntacticModifier(node, ModifierFlags.Async) &&
                !node.asteriskToken &&
                !!getImmediatelyInvokedFunctionExpression(node)) || node.kind === SyntaxKind.ClassStaticBlockDeclaration;
            ...
            if (!(currentFlow.flags & FlowFlags.Unreachable) && containerFlags & 8 /* ContainerFlags.IsFunctionLike */ && nodeIsPresent(node.body)) {
    ...
    function isNarrowableReference(expr) {
        switch (expr.kind) {
            ...
            case SyntaxKind.ElementAccessExpression:
                return (isStringOrNumericLiteralLike(expr.argumentExpression) || isEntityNameExpression(expr.argumentExpression)) &&
                    isNarrowableReference(expr.expression);
            case SyntaxKind.BinaryExpression:
                return expr.operatorToken.kind === SyntaxKind.CommaToken && isNarrowableReference(expr.right) ||
    ...
    function createBranchLabel() {
        return createFlowNode(FlowFlags.BranchLabel, /*node*/ undefined, /*antecedent*/ undefined);
    }
    function createLoopLabel() {
        return createFlowNode(FlowFlags.LoopLabel, /*node*/ undefined, /*antecedent*/ undefined);
    }
    function createReduceLabel(target, antecedents, antecedent) {
        return createFlowNode(FlowFlags.ReduceLabel, { target, antecedents }, antecedent);
    }
    ...
    function declareModuleSymbol(node) {
        ...
        const instantiated = state !== 0 /* ModuleInstanceState.NonInstantiated */;
    ...
    function bindJSDocImports() {
        ...
        for (const jsDocImportTag of jsDocImports) {
            ...
            const enclosingContainer = host ? getEnclosingContainer(host) : undefined;
            const enclosingBlockScopeContainer = host ? getEnclosingBlockScopeContainer(host) : undefined;
    ...
    function addErrorOrSuggestionDiagnostic(isError, range, message) {
        ...
        if (isError) {
            ...
            file.bindSuggestionDiagnostics = append(file.bindSuggestionDiagnostics, Object.assign(Object.assign({}, diag), { category: DiagnosticCategory.Suggestion }));
        }
    }
    function bind(node) {
        if (!node) {
                ...
                switch (specialKind) {
                    ...
                    case AssignmentDeclarationKind.Property:
                        ...
                        if (isInJSFile(node) && isIdentifier(expression)) {
                            ...
                            if (isThisInitializedDeclaration(symbol === null || symbol === void 0 ? void 0 : symbol.valueDeclaration)) {
                                bindThisPropertyAssignment(node);
                                break;
                            }
                        }
    ...
    function bindSpecialPropertyAssignment(node) {
        ...
        if (isIdentifier(rootExpr) && ((_a = lookupSymbolForName(container, rootExpr.escapedText)) === null || _a === void 0 ? void 0 : _a.flags) & SymbolFlags.Alias) {
            return;
        }
        // Fix up parent pointers since we're going to use these nodes before we bind into them
        setParent(node.left, node);
    ...
    function bindPotentiallyMissingNamespaces(namespaceSymbol, entityName, isToplevel, isPrototypeProperty, containerIsClass) {
        if ((namespaceSymbol === null || namespaceSymbol === void 0 ? void 0 : namespaceSymbol.flags) & SymbolFlags.Alias) {
    ...
    function bindTypeParameter(node) {
        ...
        if (isJSDocTemplateTag(node.parent)) {
            ...
            if (container) {
                Debug.assertNode(container, canHaveLocals);
                (_a = container.locals) !== null && _a !== void 0 ? _a : (container.locals = createSymbolTable());
            ...
            if (container) {
                Debug.assertNode(container, canHaveLocals);
                (_b = container.locals) !== null && _b !== void 0 ? _b : (container.locals = createSymbolTable());
                declareSymbol(container.locals, /*parent*/ undefined, node, SymbolFlags.TypeParameter, SymbolFlags.TypeParameterExcludes);
            }
            else {
    ...
    function shouldReportErrorOnModuleDeclaration(node) {
        ...
        return instanceState === 1 /* ModuleInstanceState.Instantiated */ || (instanceState === 2 /* ModuleInstanceState.ConstEnumOnly */ && shouldPreserveConstEnums(options));
    }
...
function isEnumDeclarationWithPreservedEmit(node, options) {
    return node.kind === SyntaxKind.EnumDeclaration && (!isEnumConst(node) || shouldPreserveConstEnums(options));
}
...
export function getContainerFlags(node) {
    switch (node.kind) {
        ...
        case SyntaxKind.Block:
            ...
            // conflict.
            return isFunctionLike(node.parent) || isClassStaticBlockDeclaration(node.parent) ? 0 /* ContainerFlags.None */ : 2 /* ContainerFlags.IsBlockScopedContainer */ | 32 /* ContainerFlags.HasLocals */;
    }
    return 0 /* ContainerFlags.None */;
}
function lookupSymbolForName(container, name) {
    var _a, _b, _c, _d, _e;
    const local = (_b = (_a = tryCast(container, canHaveLocals)) === null || _a === void 0 ? void 0 : _a.locals) === null || _b === void 0 ? void 0 : _b.get(name);
    if (local) {

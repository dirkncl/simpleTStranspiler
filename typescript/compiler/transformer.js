...
/** @internal */
export const noTransformers = { scriptTransformers: emptyArray, declarationTransformers: emptyArray };

/** @internal */
export function getTransformers(compilerOptions, customTransformers, emitOnly) {
    return {
...
export function transformNodes(resolver, host, factory, options, nodes, transformers, allowDtsFiles) {
    const enabledSyntaxKindFeatures = new Array(SyntaxKind.Count);
    ...
    const transformed = [];
    for (const node of nodes) {
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Emit, "transformNodes", node.kind === SyntaxKind.SourceFile ? { path: node.path } : { kind: node.kind, pos: node.pos, end: node.end });

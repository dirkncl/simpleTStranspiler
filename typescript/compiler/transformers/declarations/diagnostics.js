...
export function createGetIsolatedDeclarationErrors(resolver) {
    ...
    function findNearestDeclaration(node) {
        ...
        return (isStatement(result) ? undefined : result);
    }
    function createAccessorTypeError(node) {
        ...
        const targetNode = (_a = (isSetAccessor(node) ? node.parameters[0] : node)) !== null && _a !== void 0 ? _a : node;
    ...
    function addParentDeclarationRelatedInfo(node, diag) {
    ...
    function createExpressionError(node, diagnosticMessage) {
        ...
        if (parentDeclaration) {
            ...
            if (parentDeclaration === parent) {
                diag = createDiagnosticForNode(node, diagnosticMessage !== null && diagnosticMessage !== void 0 ? diagnosticMessage : errorByDeclarationKind[parentDeclaration.kind]);
                addRelatedInfo(diag, createDiagnosticForNode(parentDeclaration, relatedSuggestionByDeclarationKind[parentDeclaration.kind], targetStr));
            }
            else {
                diag = createDiagnosticForNode(node, diagnosticMessage !== null && diagnosticMessage !== void 0 ? diagnosticMessage : Diagnostics.Expression_type_can_t_be_inferred_with_isolatedDeclarations);
                addRelatedInfo(diag, createDiagnosticForNode(parentDeclaration, relatedSuggestionByDeclarationKind[parentDeclaration.kind], targetStr));

...
/** @internal */
export function computeCommonSourceDirectoryOfFilenames(fileNames, currentDirectory, getCanonicalFileName) {
...
/** @internal */
export function createGetSourceFile(readFile, setParentNodes) {
...
/** @internal */
export function createWriteFileMeasuringIO(actualWriteFile, createDirectory, directoryExists) {
...
/** @internal */
export function createCompilerHostWorker(options, setParentNodes, system = sys) {
...
/** @internal */
export function changeCompilerHostLikeToUseCache(host, toPath, getSourceFile) {
    ...
    const getSourceFileWithCache = getSourceFile ? (fileName, languageVersionOrOptions, onError, shouldCreateNewSourceFile) => {
        ...
        const value = forImpliedNodeFormat === null || forImpliedNodeFormat === void 0 ? void 0 : forImpliedNodeFormat.get(key);
        if (value)
            return value;
...
export function getPreEmitDiagnostics(program, sourceFile, cancellationToken) {
    let diagnostics;
...
export function formatDiagnostics(diagnostics, host) {
    let output = "";
...
export function formatDiagnostic(diagnostic, host) {
...
// }
/** @internal */
export var ForegroundColorEscapeSequences;
...
/** @internal */
export function formatColorAndReset(text, formatStyle) {
...
/** @internal */
export function formatLocation(file, start, host, color = formatColorAndReset) {
...
export function formatDiagnosticsWithColorAndContext(diagnostics, host) {
    let output = "";
    for (const diagnostic of diagnostics) {
        if (diagnostic.file) {
 ...
 */
export function getModeForFileReference(ref, containingFileMode) {
    return (isString(ref) ? containingFileMode : ref.resolutionMode) || containingFileMode;
}
export function getModeForResolutionAtIndex(file, index, compilerOptions) {
...
/** @internal */
export function isExclusivelyTypeOnlyImportOrExport(decl) {
    var _a;
 ...
 */
export function getModeForUsageLocation(file, usage, compilerOptions) {
...
function getEmitSyntaxForUsageLocationWorker(file, usage, compilerOptions) {
    ...
    const exprParentParent = (_a = walkUpParenthesizedExpressions(usage.parent)) === null || _a === void 0 ? void 0 : _a.parent;
...
/** @internal */
export function getResolutionModeOverride(node, grammarErrorOnNode) {
    if (!node)
        return undefined;
    if (length(node.elements) !== 1) {
        grammarErrorOnNode === null || grammarErrorOnNode === void 0 ? void 0 : grammarErrorOnNode(node, node.token === SyntaxKind.WithKeyword
    ...
    if (elem.name.text !== "resolution-mode") {
        grammarErrorOnNode === null || grammarErrorOnNode === void 0 ? void 0 : grammarErrorOnNode(elem.name, node.token === SyntaxKind.WithKeyword
    ...
    if (elem.value.text !== "import" && elem.value.text !== "require") {
        grammarErrorOnNode === null || grammarErrorOnNode === void 0 ? void 0 : grammarErrorOnNode(elem.value, Diagnostics.resolution_mode_should_be_either_require_or_import);
        return undefined;
    }
...
/** @internal */
export const moduleResolutionNameAndModeGetter = {
    getName: getModuleResolutionName,
    getMode: (entry, file, compilerOptions) => getModeForUsageLocation(file, entry, compilerOptions),
};
/** @internal */
export function createModuleResolutionLoader(containingFile, redirectedReference, options, host, cache) {
    return {
...
const typeReferenceResolutionNameAndModeGetter = {
...
/** @internal */
export function createTypeReferenceResolutionLoader(containingFile, redirectedReference, options, host, cache) {
    return {
...
/** @internal */
export function loadWithModeAwareCache(entries, containingFile, redirectedReference, options, containingSourceFile, host, resolutionCache, createLoader) {
    if (entries.length === 0)
        return emptyArray;
    const resolutions = [];
    const cache = new Map();
...
/** @internal */
export function isReferencedFile(reason) {
...
/** @internal */
export function isReferenceFileLocation(location) {
    return location.pos !== undefined;
}
/** @internal */
export function getReferencedFileLocation(program, ref) {
    var _a, _b, _c, _d;
    ...
    switch (kind) {
        case FileIncludeKind.Import:
            ...
            packageId = (_b = (_a = program.getResolvedModuleFromModuleSpecifier(importLiteral, file)) === null || _a === void 0 ? void 0 : _a.resolvedModule) === null || _b === void 0 ? void 0 : _b.packageId;
        ...
        case FileIncludeKind.TypeReferenceDirective:
            ...
            packageId = (_d = (_c = program.getResolvedTypeReferenceDirectiveFromTypeReferenceDirective(file.typeReferenceDirectives[index], file)) === null || _c === void 0 ? void 0 : _c.resolvedTypeReferenceDirective) === null || _d === void 0 ? void 0 : _d.packageId;
            break;
        case FileIncludeKind.LibReferenceDirective:
 ...
 * @internal
 */
export function isProgramUptoDate(program, rootFileNames, newOptions, getSourceVersion, fileExists, hasInvalidatedResolutions, hasInvalidatedLibResolutions, hasChangedAutomaticTypeDirectiveNames, getParsedCommandLine, projectReferences) {
...
export function getConfigFileParsingDiagnostics(configFileParseResult) {
 ...
 */
export function getImpliedNodeFormatForFile(fileName, packageJsonInfoCache, host, options) {
    const result = getImpliedNodeFormatForFileWorker(fileName, packageJsonInfoCache, host, options);
    return typeof result === "object" ? result.impliedNodeFormat : result;
}
/** @internal */
export function getImpliedNodeFormatForFileWorker(fileName, packageJsonInfoCache, host, options) {
    ...
    function lookupFromPackageJson() {
        ...
        const impliedNodeFormat = (packageJsonScope === null || packageJsonScope === void 0 ? void 0 : packageJsonScope.contents.packageJsonContent.type) === "module" ? ModuleKind.ESNext : ModuleKind.CommonJS;
        return { impliedNodeFormat, packageJsonLocations, packageJsonScope };
    }
}
const plainJSErrors = new Set([
    // binder errors
...
export function createProgram(_rootNamesOrOptions, _options, _host, _oldProgram, _configFileParsingDiagnostics) {
    ...
    tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "createProgram", { configFilePath: options.configFilePath, rootDir: options.rootDir }, /*separateBeginAndEnd*/ true);
    ...
    if (host.resolveModuleNameLiterals) {
        ...
        actualResolveModuleNamesWorker = (moduleNames, containingFile, redirectedReference, options, containingSourceFile, reusedNames) => host.resolveModuleNames(moduleNames.map(getModuleResolutionName), containingFile, reusedNames === null || reusedNames === void 0 ? void 0 : reusedNames.map(getModuleResolutionName), redirectedReference, options, containingSourceFile).map(resolved => resolved ?
            (resolved.extension !== undefined) ?
                { resolvedModule: resolved } :
                // An older host may have omitted extension, in which case we should infer it from the file extension of resolvedFileName.
                { resolvedModule: Object.assign(Object.assign({}, resolved), { extension: extensionFromPath(resolved.resolvedFileName) }) } :
            emptyResolution);
        moduleResolutionCache = (_b = host.getModuleResolutionCache) === null || _b === void 0 ? void 0 : _b.call(host);
    }
    else {
        moduleResolutionCache = createModuleResolutionCache(currentDirectory, getCanonicalFileName, options);
        actualResolveModuleNamesWorker = (moduleNames, containingFile, redirectedReference, options, containingSourceFile) => loadWithModeAwareCache(moduleNames, containingFile, redirectedReference, options, containingSourceFile, host, moduleResolutionCache, createModuleResolutionLoader);
    }
    ...
    if (host.resolveTypeReferenceDirectiveReferences) {
        ...
        actualResolveTypeReferenceDirectiveNamesWorker = (typeDirectiveNames, containingFile, redirectedReference, options, containingSourceFile) => host.resolveTypeReferenceDirectives(typeDirectiveNames.map(getTypeReferenceResolutionName), containingFile, redirectedReference, options, containingSourceFile === null || containingSourceFile === void 0 ? void 0 : containingSourceFile.impliedNodeFormat).map(resolvedTypeReferenceDirective => ({ resolvedTypeReferenceDirective }));
    }
    else {
        const typeReferenceDirectiveResolutionCache = createTypeReferenceDirectiveResolutionCache(currentDirectory, getCanonicalFileName, 
    ...
    if (host.resolveLibrary) {
        ...
        const libraryResolutionCache = createModuleResolutionCache(currentDirectory, getCanonicalFileName, options, moduleResolutionCache === null || moduleResolutionCache === void 0 ? void 0 : moduleResolutionCache.getPackageJsonInfoCache());
    ...
    let missingFileNames = new Map();
    // stores 'filename -> file association' ignoring case
    // used to track cases when two file names differ only in casing
    const filesByNameIgnoreCase = host.useCaseSensitiveFileNames() ? new Map() : undefined;
    ...
    const useSourceOfProjectReferenceRedirect = !!((_c = host.useSourceOfProjectReferenceRedirect) === null || _c === void 0 ? void 0 : _c.call(host)) &&
    ...
    tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "shouldProgramCreateNewSourceFiles", { hasOldProgram: !!oldProgram });
    ...
    tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "tryReuseStructureFromOldProgram", {});
    ...
    if (structureIsReused !== StructureIsReused.Completely) {
        ...
        if (projectReferences) {
            ...
            if (rootNames.length) {
                resolvedProjectReferences === null || resolvedProjectReferences === void 0 ? void 0 : resolvedProjectReferences.forEach((parsedRef, index) => {
                    if (!parsedRef)
                        return;
        ...
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "processRootFiles", { count: rootNames.length });
        ...
        automaticTypeDirectiveNames !== null && automaticTypeDirectiveNames !== void 0 ? automaticTypeDirectiveNames : (automaticTypeDirectiveNames = rootNames.length ? getAutomaticTypeDirectiveNames(options, host) : emptyArray);
        automaticTypeDirectiveResolutions = createModeAwareCache();
        if (automaticTypeDirectiveNames.length) {
            tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "processTypeReferences", { count: automaticTypeDirectiveNames.length });
    ...
    if (oldProgram && host.onReleaseParsedCommandLine) {
        forEachProjectReference(oldProgram.getProjectReferences(), oldProgram.getResolvedProjectReferences(), (oldResolvedRef, parent, index) => {
            const oldReference = (parent === null || parent === void 0 ? void 0 : parent.commandLine.projectReferences[index]) || oldProgram.getProjectReferences()[index];
    ...
    function getResolvedModule(file, moduleName, mode) {
        var _a;
        return (_a = resolvedModules === null || resolvedModules === void 0 ? void 0 : resolvedModules.get(file.path)) === null || _a === void 0 ? void 0 : _a.get(moduleName, mode);
    }
    function getResolvedModuleFromModuleSpecifier(moduleSpecifier, sourceFile) {
        sourceFile !== null && sourceFile !== void 0 ? sourceFile : (sourceFile = getSourceFileOfNode(moduleSpecifier));
    ...
    function getResolvedTypeReferenceDirective(file, typeDirectiveName, mode) {
        var _a;
        return (_a = resolvedTypeReferenceDirectiveNames === null || resolvedTypeReferenceDirectiveNames === void 0 ? void 0 : resolvedTypeReferenceDirectiveNames.get(file.path)) === null || _a === void 0 ? void 0 : _a.get(typeDirectiveName, mode);
    }
    ...
    function forEachResolution(resolutionCache, callback, file) {
        var _a;
        if (file)
            (_a = resolutionCache === null || resolutionCache === void 0 ? void 0 : resolutionCache.get(file.path)) === null || _a === void 0 ? void 0 : _a.forEach((resolution, name, mode) => callback(resolution, name, mode, file.path));
        else
            resolutionCache === null || resolutionCache === void 0 ? void 0 : resolutionCache.forEach((resolutions, filePath) => resolutions.forEach((resolution, name, mode) => callback(resolution, name, mode, filePath)));
    }
    ...
    function resolveModuleNamesWorker(moduleNames, containingFile, reusedNames) {
        const containingFileName = getNormalizedAbsolutePath(containingFile.originalFileName, currentDirectory);
        const redirectedReference = getRedirectReferenceForResolution(containingFile);
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "resolveModuleNamesWorker", { containingFileName });
    ...
    function resolveTypeReferenceDirectiveNamesWorker(typeDirectiveNames, containingFile, reusedNames) {
        ...
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "resolveTypeReferenceDirectiveNamesWorker", { containingFileName });
    ...
    function getCommonSourceDirectory() {
        ...
        commonSourceDirectory = ts_getCommonSourceDirectory(options, () => mapDefined(emittedFiles, file => file.isDeclarationFile ? undefined : file.fileName), currentDirectory, getCanonicalFileName, commonSourceDirectory => checkSourceFilesBelongToPath(emittedFiles, commonSourceDirectory));
    ...
    function getClassifiableNames() {
        ...
        if (!classifiableNames) {
            ...
            for (const sourceFile of files) {
                (_a = sourceFile.classifiableNames) === null || _a === void 0 ? void 0 : _a.forEach(value => classifiableNames.add(value));
            }
        }
    ...
    function resolveModuleNamesReusingOldState(moduleNames, containingFile) {
        return resolveNamesReusingOldState({
            ...
            getResolutionFromOldProgram: (name, mode) => oldProgram === null || oldProgram === void 0 ? void 0 : oldProgram.getResolvedModule(containingFile, name, mode),
            getResolved: getResolvedModuleFromResolution,
            canReuseResolutionsInFile: () => containingFile === (oldProgram === null || oldProgram === void 0 ? void 0 : oldProgram.getSourceFile(containingFile.fileName)) &&
    ...
    function resolveTypeReferenceDirectiveNamesReusingOldState(typeDirectiveNames, containingFile) {
        ...
        return resolveNamesReusingOldState({
            ...
            getResolutionFromOldProgram: (name, mode) => {
                var _a;
                return containingSourceFile ?
                    oldProgram === null || oldProgram === void 0 ? void 0 : oldProgram.getResolvedTypeReferenceDirective(containingSourceFile, name, mode) :
                    (_a = oldProgram === null || oldProgram === void 0 ? void 0 : oldProgram.getAutomaticTypeDirectiveResolutions()) === null || _a === void 0 ? void 0 : _a.get(name, mode);
            },
            getResolved: getResolvedTypeReferenceDirectiveFromResolution,
            canReuseResolutionsInFile: () => containingSourceFile ?
                containingSourceFile === (oldProgram === null || oldProgram === void 0 ? void 0 : oldProgram.getSourceFile(containingSourceFile.fileName)) && !hasInvalidatedResolutions(containingSourceFile.path) :
                !hasInvalidatedResolutions(toPath(containingFile)),
        });
    }
    function resolveNamesReusingOldState({ entries, containingFile, containingSourceFile, redirectedReference, nameAndModeGetter, resolutionWorker, getResolutionFromOldProgram, getResolved, canReuseResolutionsInFile, resolveToOwnAmbientModule, }) {
        var _a;
        if (!entries.length)
            return emptyArray;
        if (structureIsReused === StructureIsReused.Not && (!resolveToOwnAmbientModule || !containingSourceFile.ambientModuleNames.length)) {
            ...
            return resolutionWorker(entries, containingFile, 
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            if (reuseResolutions) {
                ...
                const mode = nameAndModeGetter.getMode(entry, containingSourceFile, (_a = redirectedReference === null || redirectedReference === void 0 ? void 0 : redirectedReference.commandLine.options) !== null && _a !== void 0 ? _a : options);
                const oldResolution = getResolutionFromOldProgram(name, mode);
                const oldResolved = oldResolution && getResolved(oldResolution);
                if (oldResolved) {
                    if (isTraceEnabled(options, host)) {
                        trace(host, resolutionWorker === resolveModuleNamesWorker ?
                            ...
                            oldResolved.packageId ?
                                ...
                                Diagnostics.Reusing_resolution_of_type_reference_directive_0_from_1_of_old_program_it_was_successfully_resolved_to_2, name, containingSourceFile ? getNormalizedAbsolutePath(containingSourceFile.originalFileName, currentDirectory) : containingFile, oldResolved.resolvedFileName, oldResolved.packageId && packageIdToString(oldResolved.packageId));
                    }
                    (result !== null && result !== void 0 ? result : (result = new Array(entries.length)))[i] = oldResolution;
            ...
            if (resolveToOwnAmbientModule) {
                ...
                if (contains(containingSourceFile.ambientModuleNames, name)) {
                    ...
                    (result !== null && result !== void 0 ? result : (result = new Array(entries.length)))[i] = emptyResolution;
                    continue;
                }
            }
            // Resolution failed in the old program, or resolved to an ambient module for which we can't reuse the result.
            (unknownEntries !== null && unknownEntries !== void 0 ? unknownEntries : (unknownEntries = [])).push(entry);
            (unknownEntryIndices !== null && unknownEntryIndices !== void 0 ? unknownEntryIndices : (unknownEntryIndices = [])).push(i);
        }
        if (!unknownEntries)
            return result;
    ...
    function tryReuseStructureFromOldProgram() {
        ...
        for (const oldSourceFile of oldSourceFiles) {
            ...
            if (packageName !== undefined) {
                ...
                const newKind = fileChanged ? 1 /* SeenPackageName.Modified */ : 0 /* SeenPackageName.Exists */;
        ...
        for (const newSourceFile of modifiedSourceFiles) {
            ...
            (resolvedModulesProcessing !== null && resolvedModulesProcessing !== void 0 ? resolvedModulesProcessing : (resolvedModulesProcessing = new Map())).set(newSourceFile.path, resolutions);
            const optionsForFile = getCompilerOptionsForFile(newSourceFile);
            // ensure that module resolution results are still correct
            const resolutionsChanged = hasChangesInResolutions(moduleNames, resolutions, name => oldProgram.getResolvedModule(newSourceFile, name.text, getModeForUsageLocationWorker(newSourceFile, name, optionsForFile)), moduleResolutionIsEqualTo);
            if (resolutionsChanged)
                structureIsReused = StructureIsReused.SafeModules;
            const typesReferenceDirectives = newSourceFile.typeReferenceDirectives;
            const typeReferenceResolutions = resolveTypeReferenceDirectiveNamesReusingOldState(typesReferenceDirectives, newSourceFile);
            (resolvedTypeReferenceDirectiveNamesProcessing !== null && resolvedTypeReferenceDirectiveNamesProcessing !== void 0 ? resolvedTypeReferenceDirectiveNamesProcessing : (resolvedTypeReferenceDirectiveNamesProcessing = new Map())).set(newSourceFile.path, typeReferenceResolutions);
            // ensure that types resolutions are still correct
            const typeReferenceResolutionsChanged = hasChangesInResolutions(typesReferenceDirectives, typeReferenceResolutions, name => oldProgram.getResolvedTypeReferenceDirective(newSourceFile, getTypeReferenceResolutionName(name), getModeForTypeReferenceDirectiveInFile(name, newSourceFile)), typeDirectiveIsEqualTo);
    ...
    function getEmitHost(writeFileCallback) {
        return {
            ...
            useCaseSensitiveFileNames: () => host.useCaseSensitiveFileNames(),
            getBuildInfo: () => { var _a; return (_a = program.getBuildInfo) === null || _a === void 0 ? void 0 : _a.call(program); },
    ...
    function emitBuildInfo(writeFileCallback) {
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Emit, "emitBuildInfo", {}, /*separateBeginAndEnd*/ true);
    ...
    function emit(sourceFile, writeFileCallback, cancellationToken, emitOnly, transformers, forceDtsEmit, skipBuildInfo) {
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Emit, "emit", { path: sourceFile === null || sourceFile === void 0 ? void 0 : sourceFile.path }, /*separateBeginAndEnd*/ true);
        const result = runWithCancellationToken(() => emitWorker(program, sourceFile, writeFileCallback, cancellationToken, emitOnly, transformers, forceDtsEmit, skipBuildInfo));
        tracing === null || tracing === void 0 ? void 0 : tracing.pop();
        return result;
    }
    ...
    function emitWorker(program, sourceFile, writeFileCallback, cancellationToken, emitOnly, customTransformers, forceDtsEmit, skipBuildInfo) {
        if (!forceDtsEmit) {
        ...
        const emitResolver = typeChecker.getEmitResolver(options.outFile ? undefined : sourceFile, cancellationToken, emitResolverSkipsTypeChecking(emitOnly, forceDtsEmit));
        performance.mark("beforeEmit");
        const emitResult = typeChecker.runWithCancellationToken(cancellationToken, () => emitFiles(emitResolver, getEmitHost(writeFileCallback), sourceFile, getTransformers(options, customTransformers, emitOnly), emitOnly, 
        /*onlyBuildInfo*/ false, forceDtsEmit, skipBuildInfo));
        performance.mark("afterEmit");
        performance.measure("Emit", "beforeEmit", "afterEmit");
        return emitResult;
    }
    ...
    function getSemanticDiagnostics(sourceFile, cancellationToken, nodesToCheck) {
        return getDiagnosticsHelper(sourceFile, (sourceFile, cancellationToken) => getSemanticDiagnosticsForFile(sourceFile, cancellationToken, nodesToCheck), cancellationToken);
    }
    function getCachedSemanticDiagnostics(sourceFile) {
        return cachedBindAndCheckDiagnosticsForFile === null || cachedBindAndCheckDiagnosticsForFile === void 0 ? void 0 : cachedBindAndCheckDiagnosticsForFile.get(sourceFile.path);
    }
    ...
    function getBindAndCheckDiagnosticsForFile(sourceFile, cancellationToken, nodesToCheck) {
        ...
        let result = cachedBindAndCheckDiagnosticsForFile === null || cachedBindAndCheckDiagnosticsForFile === void 0 ? void 0 : cachedBindAndCheckDiagnosticsForFile.get(sourceFile.path);
        if (!result) {
            (cachedBindAndCheckDiagnosticsForFile !== null && cachedBindAndCheckDiagnosticsForFile !== void 0 ? cachedBindAndCheckDiagnosticsForFile : (cachedBindAndCheckDiagnosticsForFile = new Map())).set(sourceFile.path, result = getBindAndCheckDiagnosticsForFileNoCache(sourceFile, cancellationToken));
        }
        return result;
    }
    ...
    function getJSSyntacticDiagnosticsForFile(sourceFile) {
        return runWithCancellationToken(() => {
            ...
            function createDiagnosticForNodeArray(nodes, message, ...args) {
    ...
    function getDeclarationDiagnosticsWorker(sourceFile, cancellationToken) {
        let result = cachedDeclarationDiagnosticsForFile === null || cachedDeclarationDiagnosticsForFile === void 0 ? void 0 : cachedDeclarationDiagnosticsForFile.get(sourceFile.path);
        if (!result) {
            (cachedDeclarationDiagnosticsForFile !== null && cachedDeclarationDiagnosticsForFile !== void 0 ? cachedDeclarationDiagnosticsForFile : (cachedDeclarationDiagnosticsForFile = new Map())).set(sourceFile.path, result = getDeclarationDiagnosticsForFileNoCache(sourceFile, cancellationToken));
        }
        return result;
    }
    ...
    function createSyntheticImport(text, file) {
        ...
        externalHelpersModuleReference.flags &= ~NodeFlags.Synthesized;
        importDecl.flags &= ~NodeFlags.Synthesized;
    ...
    function getLibFileFromReference(ref) {
        ...
        const actualFileName = libFileName && ((_a = resolvedLibReferences === null || resolvedLibReferences === void 0 ? void 0 : resolvedLibReferences.get(libFileName)) === null || _a === void 0 ? void 0 : _a.actual);
    ...
    function findSourceFile(fileName, isDefaultLib, ignoreNoDefaultLib, reason, packageId) {
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "findSourceFile", {
            fileName,
    ...
    function getCreateSourceFileOptions(fileName, moduleResolutionCache, host, options) {
        ...
        const result = getImpliedNodeFormatForFileWorker(getNormalizedAbsolutePath(fileName, currentDirectory), moduleResolutionCache === null || moduleResolutionCache === void 0 ? void 0 : moduleResolutionCache.getPackageJsonInfoCache(), host, options);
        const languageVersion = getEmitScriptTarget(options);
        const setExternalModuleIndicator = getSetExternalModuleIndicator(options);
        return typeof result === "object" ? Object.assign(Object.assign({}, result), { languageVersion, setExternalModuleIndicator, jsDocParsingMode: host.jsDocParsingMode }) :
    ...
    function findSourceFileWorker(fileName, isDefaultLib, ignoreNoDefaultLib, reason, packageId) {
        ...
        if (useSourceOfProjectReferenceRedirect) {
            ...
            if (source) {
                const file = isString(source) ?
                    ...
                    undefined;
                if (file)
                    addFileToFilesByName(file, path, fileName, /*redirectedPath*/ undefined);
                return file;
            }
        }
        ...
        const file = host.getSourceFile(fileName, sourceFileOptions, hostErrorMessage => addFilePreprocessingFileExplainingDiagnostic(/*file*/ undefined, reason, Diagnostics.Cannot_read_file_0_Colon_1, [fileName, hostErrorMessage]), shouldCreateNewSourceFile);
        if (packageId) {
        ...
        if (file) {
            ...
            (filesWithReferencesProcessed !== null && filesWithReferencesProcessed !== void 0 ? filesWithReferencesProcessed : (filesWithReferencesProcessed = new Set())).add(file.path);
        }
        return file;
    }
    function addFileIncludeReason(file, reason, checkExisting) {
        if (file && (!checkExisting || !isReferencedFile(reason) || !(filesWithReferencesProcessed === null || filesWithReferencesProcessed === void 0 ? void 0 : filesWithReferencesProcessed.has(reason.file)))) {
    ...
    function processTypeReferenceDirectives(file) {
        ...
        if (!typeDirectives.length)
            return;
        const resolutions = (resolvedTypeReferenceDirectiveNamesProcessing === null || resolvedTypeReferenceDirectiveNamesProcessing === void 0 ? void 0 : resolvedTypeReferenceDirectiveNamesProcessing.get(file.path)) ||
            resolveTypeReferenceDirectiveNamesReusingOldState(typeDirectives, file);
        const resolutionsInFile = createModeAwareCache();
        (resolvedTypeReferenceDirectiveNames !== null && resolvedTypeReferenceDirectiveNames !== void 0 ? resolvedTypeReferenceDirectiveNames : (resolvedTypeReferenceDirectiveNames = new Map())).set(file.path, resolutionsInFile);
    ...
    function getCompilerOptionsForFile(file) {
        var _a;
        return ((_a = getRedirectReferenceForResolution(file)) === null || _a === void 0 ? void 0 : _a.commandLine.options) || options;
    }
    function processTypeReferenceDirective(typeReferenceDirective, mode, resolution, reason) {
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "processTypeReferenceDirective", { directive: typeReferenceDirective, hasResolved: !!resolution.resolvedTypeReferenceDirective, refKind: reason.kind, refPath: isReferencedFile(reason) ? reason.file : undefined });
    ...
    function pathForLibFile(libFileName) {
        const existing = resolvedLibReferences === null || resolvedLibReferences === void 0 ? void 0 : resolvedLibReferences.get(libFileName);
        if (existing)
    ...
    function pathForLibFileWorker(libFileName) {
        var _a, _b, _c;
        const existing = resolvedLibProcessing === null || resolvedLibProcessing === void 0 ? void 0 : resolvedLibProcessing.get(libFileName);
        if (existing)
            return existing;
        ...
        if (structureIsReused !== StructureIsReused.Not && oldProgram && !hasInvalidatedLibResolutions(libFileName)) {
            const oldResolution = (_a = oldProgram.resolvedLibReferences) === null || _a === void 0 ? void 0 : _a.get(libFileName);
            if (oldResolution) {
                if (oldResolution.resolution && isTraceEnabled(options, host)) {
                    ...
                    trace(host, oldResolution.resolution.resolvedModule ?
                        oldResolution.resolution.resolvedModule.packageId ?
                            Diagnostics.Reusing_resolution_of_module_0_from_1_of_old_program_it_was_successfully_resolved_to_2_with_Package_ID_3 :
                            Diagnostics.Reusing_resolution_of_module_0_from_1_of_old_program_it_was_successfully_resolved_to_2 :
                        Diagnostics.Reusing_resolution_of_module_0_from_1_of_old_program_it_was_not_resolved, libraryName, getNormalizedAbsolutePath(resolveFrom, currentDirectory), (_b = oldResolution.resolution.resolvedModule) === null || _b === void 0 ? void 0 : _b.resolvedFileName, ((_c = oldResolution.resolution.resolvedModule) === null || _c === void 0 ? void 0 : _c.packageId) && packageIdToString(oldResolution.resolution.resolvedModule.packageId));
                }
        ...
        const libraryName = getLibraryNameFromLibFileName(libFileName);
        const resolveFrom = getInferredLibraryNameResolveFrom(options, currentDirectory, libFileName);
        tracing === null || tracing === void 0 ? void 0 : tracing.push(tracing.Phase.Program, "resolveLibrary", { resolveFrom });
    ...
    function processImportedModules(file) {
        ...
        if (file.imports.length || file.moduleAugmentations.length) {
            ...
            const resolutions = (resolvedModulesProcessing === null || resolvedModulesProcessing === void 0 ? void 0 : resolvedModulesProcessing.get(file.path)) ||
                resolveModuleNamesReusingOldState(moduleNames, file);
            Debug.assert(resolutions.length === moduleNames.length);
            const optionsForFile = getCompilerOptionsForFile(file);
            const resolutionsInFile = createModeAwareCache();
            (resolvedModules !== null && resolvedModules !== void 0 ? resolvedModules : (resolvedModules = new Map())).set(file.path, resolutionsInFile);
    ...
    function verifyCompilerOptions() {
        ...
        else if (ModuleResolutionKind[moduleResolution] &&
            (ModuleResolutionKind.Node16 <= moduleResolution && moduleResolution <= ModuleResolutionKind.NodeNext) &&
            !(ModuleKind.Node16 <= moduleKind && moduleKind <= ModuleKind.NodeNext)) {
    ...
    function createDiagnosticForOptionPaths(onKey, key, message, ...args) {
        ...
        forEachOptionPathsSyntax(pathProp => {
            if (isObjectLiteralExpression(pathProp.initializer) &&
                createOptionDiagnosticInObjectLiteralSyntax(pathProp.initializer, onKey, key, 
                /*key2*/ undefined, message, ...args)) {
                needCompilerDiagnostic = false;
            }
        });
        if (needCompilerDiagnostic) {
    ...
    function isEmittedFile(file) {
        ...
        if (fileExtensionIsOneOf(filePath, supportedJSExtensionsFlat) || isDeclarationFileName(filePath)) {
            ...
            return !!getSourceFileByPath((filePathWithoutExtension + Extension.Ts)) ||
                !!getSourceFileByPath((filePathWithoutExtension + Extension.Tsx));
        }
        return false;
    }
    ...
    function getDefaultResolutionModeForFile(sourceFile) {
...
export function getEmitModuleFormatOfFileWorker(sourceFile, options) {
    var _a;
    return (_a = getImpliedNodeFormatForEmitWorker(sourceFile, options)) !== null && _a !== void 0 ? _a : getEmitModuleKind(options);
}
/** @internal Prefer `program.getImpliedNodeFormatForEmit` when possible. */
export function getImpliedNodeFormatForEmitWorker(sourceFile, options) {
    var _a, _b;
    const moduleKind = getEmitModuleKind(options);
    ...
    if (sourceFile.impliedNodeFormat === ModuleKind.CommonJS
        && (((_a = sourceFile.packageJsonScope) === null || _a === void 0 ? void 0 : _a.contents.packageJsonContent.type) === "commonjs"
            || fileExtensionIsOneOf(sourceFile.fileName, [Extension.Cjs, Extension.Cts]))) {
        return ModuleKind.CommonJS;
    }
    if (sourceFile.impliedNodeFormat === ModuleKind.ESNext
        && (((_b = sourceFile.packageJsonScope) === null || _b === void 0 ? void 0 : _b.contents.packageJsonContent.type) === "module"
...
export function getDefaultResolutionModeForFileWorker(sourceFile, options) {
...
function updateHostForUseSourceOfProjectReferenceRedirect(host) {
    ...
    function fileOrDirectoryExistsUsingSource(fileOrDirectory, isFile) {
        ...
        const fileOrDirectoryExistsUsingSource = isFile ?
            (file) => fileExistsIfProjectReferenceDts(file) :
            (dir) => directoryExistsIfProjectReferenceDeclDir(dir);
...
/** @internal */
export const emitSkippedWithNoDiagnostics = { diagnostics: emptyArray, sourceMaps: undefined, emittedFiles: undefined, emitSkipped: true };
/** @internal */
export function handleNoEmitOptions(program, sourceFile, writeFile, cancellationToken) {
...
/** @internal */
export function filterSemanticDiagnostics(diagnostic, option) {
    return filter(diagnostic, d => !d.skippedOn || !option[d.skippedOn]);
}
/** @internal */
export function parseConfigHostFromCompilerHostLike(host, directoryStructureHost = host) {
    return {
 ...
 * @internal
 */
export function getResolutionDiagnostic(options, { extension }, { isDeclarationFile }) {
    switch (extension) {
        case Extension.Ts:
        case Extension.Dts:
        case Extension.Mts:
...
/** @internal */
export function getModuleNameStringLiteralAt({ imports, moduleAugmentations }, index) {
